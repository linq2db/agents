...
Then re-run step 4 with `-UpdateBaseline -BuildCommand '<the exact command from step 3>'`. Without the write-back the next rule's run has nothing to diff and the regression half of this mode silently degrades to "here is a number". The baseline is a corpus file — the write is a `.claude/` submodule commit pushed to the agents repo, never onto a linq2db branch.
