...
**A *green* check can belong to a superseded commit — date the build against the PR's commits before crediting it.** `gh pr checks` lists whatever checks are attached to the PR, and a passing `test-all` from an earlier run stays visible after new commits land. Compare the build's start time against `commits[*].date` from step 2's context: when the newest fix commit postdates the green run, **no completed CI run has validated the code under review**, so any finding that leans on "CI is green" is unfounded and the neutrality claims in the PR body are unverified. Record it in `## Review notes` rather than treating the check as coverage. This is the CI-check counterpart of the baselines-branch dating rule in step 4 Q2 — same failure, different artifact. (Surfaced on #5801: `test-all` was green on build 22915, which started 2026-08-19 12:46 UTC and covered only `8a202ab`, the branch's first and test-only commit; all three commits carrying the actual fix postdated it, the build for the reviewed HEAD was still running, and one of those commits' own messages recorded that an earlier version of the change had moved a baseline.)
...
**`pr.state` from the context load can be stale — sanity-check it before treating the PR as open.** The whole merged-PR branch below is gated on one field, and if that field is wrong the branch silently never fires: no delivery question, no frozen-body warning, no baselines-pruned note, and the review runs as a change request against code that already shipped. The cheap check is **internal consistency**: `state: OPEN` together with `isDraft: true` is impossible for a PR that has been merged, so the pair reading as open-and-draft on a PR whose commits are days old is a tell, not a fact. Confirm with `gh pr view <n> --repo linq2db/linq2db --json state,mergedAt,mergeCommit --jq '{state,mergedAt,mergeCommit:.mergeCommit.oid}'` before step 4's gate — one call, and it also gives you the merge commit the merged-PR path needs. (Surfaced on #5820: the context load reported `OPEN` / draft for a PR merged four hours earlier; the mismatch only came to light when the user said *"pr already merged"* mid-walk, after the scope gate and the reviewers had already run.)
...
Two mechanical consequences for the steps below. **Step 4's question 2 is moot** — CI prunes `baselines/pr_<n>` once the tracking baselines PR is merged/closed, so report *"baselines branch pruned on merge — pass unavailable"* rather than offering the opt-out, and skip the presence/age probes. **Step 4 gains a delivery question**, because a PENDING draft review is usually the wrong vehicle on a merged PR — nobody reopens its Files tab. Ask it alongside the scope question: **report-only** (no GitHub write; the sensible default for a retrospective pass), **issue comments** on the PR thread (where the conversation already is, and what a maintainer will actually see), or **a draft review** when the user wants findings line-anchored anyway. Frame anything posted as a *post-merge note*, not a change request. (Surfaced on #5754, reviewed a day after merge: the delivery question had to be improvised, and the walk then posted four thread comments.)
...
**A missing branch has three readings, not two — check the tracking PR's state before concluding "no baselines".** `baselines-pr-scan.ps1` reports only `status: branch_missing`, which is the same output for all of them: **never created** (no baselines), **created then closed** (found empty), and **MERGED** — the branch is deleted on merge, so its delta now lives in baselines *master* and the pass is still reviewable, just from the merge commit rather than a branch diff. The third case happens while the source PR is **still open**, so don't read a merged tracking PR as "the source PR already landed". Review it via `gh pr view <n> --repo linq2db/linq2db.baselines --json mergeCommit,additions,deletions,changedFiles` then `git -C ../linq2db.baselines show --stat <sha>` (and `show <sha> -- '<glob>'` for a sample), and render `## Baselines` off that instead of the compare view.

**Finding the tracking PR: enumerate and filter on `headRefName`, don't `--search`.** `gh pr list --repo linq2db/linq2db.baselines --search "pr_<n>"` returns `[]` even when the PR exists — the branch-derived title/ref isn't matched by the search index. Use `gh pr list --repo linq2db/linq2db.baselines --state all --limit 30 --json number,title,state,headRefName --jq '.[] | select(.headRefName == "baselines/pr_<n>")'`. (Surfaced on #5761: `--search` and `gh search prs` both came back empty for `linq2db.baselines#2052`, which was MERGED with the source PR still open.)

**Dating the branch against PR HEAD is necessary but not sufficient — check for a newer *completed build* too.** The rule below compares the branch's newest commit against the PR's head commit, and a branch that is *newer* passes it. That says nothing about whether a **later CI run** has since regenerated the branch: the baselines branch is force-recreated per run, so a delta can pass the date check at review time and be wholly superseded hours later, including by a run triggered while the review is still open. Before writing the Baselines section, list the PR's builds (`gh api "https://dev.azure.com/linq2db/linq2db/_apis/build/builds?definitions=4&api-version=7.0&%24top=5" --jq '.value[] | {id,status,result,queueTime,pr:.triggerInfo["pr.number"],sha:.triggerInfo["pr.sourceSha"]}'`) and re-fetch the branch if a completed run postdates the commits you dated against. The same call reveals a PR head that has moved since the context load — `pr.sourceSha` is the head CI actually built. **And check it before triggering a run of your own**, or you spend an hour of shared CI reproducing what already exists. (Surfaced on #5737: the tip-vs-HEAD check passed — branch 15:46 UTC against a 13:19 UTC head — and the delta was still stale. A `test-all` the following evening took it from 217 files / 47 net-positive to 167 files / **zero** net-positive, and an `/azp run test-all` was fired before anyone looked for that build.)

**A *present* branch can still be too old to review — date it against PR HEAD before recommending `y`.** The baselines branch is written by CI, so any commit pushed after the last completed run leaves it describing **superseded** SQL. Compare the branch's newest commit date (`git -C ../linq2db.baselines log -1 --format="%cI" origin/baselines/pr_<n>`) against the PR's HEAD commit date (`commits[0].date` from step 2's context). When HEAD is newer *and* any of the intervening commits touches SQL generation, recommend **defer**: the pass would diff output the current code no longer produces, and its findings can be ones those very commits already fixed. Say so in the Q2 prompt rather than asking a bare y/n, and if the user defers, render `## Baselines` with the date evidence (branch tip, HEAD tip, the SQL-affecting commit) instead of the bare "skipped per user request" line — a future reader needs to know the pass is *outstanding*, not declined. Re-run it once the in-flight CI finishes. This is distinct from the two staleness cases in step 8 rule 4: the branch being **force-recreated mid-review** (#5468) and the **PR branch lagging master** (#5605). Here the branch is simply older than the PR's own head, which is the ordinary state whenever someone pushes after CI ran. (Surfaced on PR #5732: the branch stopped at 2026-07-30 03:50 UTC while HEAD `9b387b5fd` was 13 hours newer, with `25d80dadb` — which removed a redundant sequence `ORDER BY` — in between; reviewing it would have re-raised the `as [cN]` duplicate-sort-key shape that commit had already fixed.)
...
**Sweep your own briefing text for named files, and cache the out-of-diff ones with `include.content: true`.** The rule above covers *changed* files; the bigger overage comes from questions you pose that can only be answered from files the PR doesn't touch — "does test X's `[DataSources]` include LinqService", "is `BUGCHECK` defined for the CI configuration", "does the serializer carry a version token". Each such question costs the pass a `git grep`-then-`git show` pair it must improvise, and a wrong path guess costs two. Before writing the manifest, re-read each briefing and add every file it names by path or by symbol; guessing paths yourself is cheaper here than in the pass, because a miss just leaves the entry absent rather than burning a round-trip. (Surfaced on PR #5723: the `api-and-test` pass spent ~29 spot calls against its 0–3 budget, essentially all of them fetching `StringJoinTests.cs`, `Tests/Base/Attributes/DataSourcesAttribute.cs`, `LinqToDB.csproj` and `Build/Azure/pipelines/templates/build-vars.yml` — every one named or implied by the briefing, none in the 25-file diff, and two reached only after a failed path guess.)

**For a provider-matrix PR, the out-of-diff companions are predictable — cache them without waiting to be asked.** When the diff touches N per-provider `*SqlExpressionConvertVisitor.cs` / translator files, the questions a `sql-and-provider` pass must answer ("what DDL does this `DataType` render as here?", "what precedence does this operator get by default?", "does this provider's builder emit `timestamp(7)`?") are answerable only from files the PR does *not* touch: each affected provider's `*SqlBuilderBase.cs` / `*SqlBuilder.cs`, and `Source/LinqToDB/SqlQuery/Precedence.cs`. Add them to the prep manifest alongside the diff's own files. (Surfaced on #5750, a 14-provider PR: the pass spent ~24 shell calls against its 0–3 budget, and the load-bearing evidence for two findings came from `OracleSqlBuilderBase.cs`, `DB2SqlBuilderBase.cs` and `Precedence.cs` — none in the 69-file diff, several reached only after a failed `Glob`, and most fetched with `git grep` / `git ls-files` where `Grep` / `Glob` were the correct prompt-free tools.)

**For an expression-pipeline / query-cache PR the companions are equally predictable.** When the diff touches `Internal/Linq/Builder/*` — an optimizer visitor, `ExpressionBuildVisitor`, `AssociationHelper`, a cache-keying path — the questions a pass must answer ("what else did that optimizer do?", "what does `ClearCaches` actually clear?", "is the closure folded at expose time or build time?", "does the cache key see this constant?") are answerable only from files the PR does *not* touch: `Internal/Expressions/InternalExtensions.cs` (the `OptimizeExpression` extension **and** `ExpressionOptimizerVisitor`, and note `Unwrap` lives here too), `Internal/Linq/Query.cs`, `Internal/Linq/Query{T}.cs`, `Internal/Linq/QueryCache.cs`, and `Internal/Linq/Builder/Visitors/ExposeExpressionVisitor.cs`. Add them with `include.content: true`. Beware the near-miss path `Internal/Expressions/ExpressionExtensions.cs`, which does **not** exist — guessing it wastes a manifest slot silently (`exists: false`). (Surfaced on #5801: the pass spent ~24 git calls against its 0–3 budget; five were `git show` dumps of exactly these files, and roughly ten more were failed path guesses.)

**When such a PR's own tests assert *cache* behaviour, the test harness is a second companion set — and it is the one the `api-and-test` pass burns its budget on.** The `Source/` list above answers "what does the product do"; it cannot answer "can this test fail", which is the highest-value question a cache-test reviewer asks. Deciding it needs `Tests/Base/Attributes/QueryCacheTestAttribute.cs` (whether the attribute clears the cache or merely lifts `MaxEntriesOverride` — it lifts, deliberately, so `[Values(1, 2)] int iteration` cases share entries), `Tests/Base/QueryUtils.cs` (`GetCacheMissCount`'s scope — a process-global counter per element type), and `Tests/Base/TestBase.AssertQuery.cs` (whether the expected leg re-executes the real client-side method). Add whichever prior-art fixture the PR's tests resemble too — `Tests/Linq/UserTests/Issue5302Tests.cs` and `Tests/Linq/DataProvider/PostgreSQLArrayTests.cs` are the recurring ones for extension-argument caching — plus the mapping schema of any provider the tests pin, since "is this type scalar here" is a per-provider fact (`Internal/DataProvider/PostgreSQL/PostgreSQLMappingSchema.cs` for the `AddScalarArray` registrations). (Surfaced on #5841: the `api-and-test` pass ran ~32 shell calls against its 0–3 budget, and essentially every one fetched a file from this list.)
...
**The gate is about context pressure, so weigh diff *size*, not just file count.** A PR can clear the >5 threshold while being trivially small — a one-line change repeated across eight files. Fan out to three passes only when the diff is genuinely large enough to strain one pass: use **> 5 changed files *and* (> ~150 changed lines **or** a `Source/` surface spanning more than one subsystem)**. Below that, one `focus: "all"` pass covers it at a third of the cost. State the choice when you deviate from the raw count, so the reader knows it was deliberate. (Surfaced on #5754: 8 files / 45 changed lines / zero `Source/` surface — three passes would have been two-thirds waste, and Pass B's `sql-and-provider` charter was empty regardless.)

**A PR with no source surface can be reviewed directly.** When the PR touches no `.cs` / `.fs` / `.vb` source or test files (pure `.csproj` / `.props` / `.targets` / `.md` / `.yml` — packaging, build wiring, docs, CI), the `code-reviewer`'s C#/SQL rubric adds little; reviewing directly in the main thread is acceptable and cheaper than a subagent round-trip. Still verify the domain-specific correctness yourself: package layout, Roslyn/TFM version-folder targeting, `GetTargetPath`/multi-TFM MSBuild behavior, analyzer doc-page accuracy (step 3d), YAML validity. Reviewing directly replaces **only** the subagent spawn — steps 7b (out-of-scope disposition), 8 (body assembly), and **9 (mode-choice gate — `interactive` is the default: offer submit-all / interactive / cancel, and in interactive mode walk items one at a time)** still apply. Don't collapse a direct review into a single batched findings summary + batched question. (Surfaced on #5720, a 3-file analyzer-bundling packaging PR — the direct-review path skipped the mode-choice gate and batched the findings.)

**In a direct/delegated review, verify PR-*modified* files against PR HEAD, not the working tree.** When you — or a delegated `Explore` / `general-purpose` agent — check a claim by reading a file directly (rather than through the `code-reviewer`, which reads the PR-HEAD diff cache), read PR-modified files from **PR HEAD**: the diff cache populated with `include.content: true`, or `git ls-tree <ref> <path>` + `git cat-file -p <blob>`. The review clone usually sits on a *different branch* than the PR, so a modified file's **working-tree** copy is the pre-PR version and yields false verdicts. Working-tree reads are safe only for files the PR *references but doesn't change* (product source/scripts/config the docs describe). Tip: set `content: true` in the diff-prep manifest up front so you can also run tooling (`node --check`, a linter) against HEAD without a separate `cat-file`. (Surfaced on #5721, a docs-only pass reviewed from `infra/agents-curation`: a delegated verifier reported the `track-kb-usage.js` hook fix "not implemented" because it read the pre-PR working-tree copy; the PR-HEAD blob did implement it.)
...
**When `currentUser`'s prior review was delivered as pushed commits rather than a draft review, restate its settled ground in every briefing yourself.** On this repo that is the *common* case, not an edge one — the standing preference is fix-and-push over post ([`review-orchestration.md`](../../docs/review-orchestration.md) → *Default the recommendation to a pushed fix*), and a walk where every item is fixed posts no review at all. The prior findings then live in an `issueComments[]` entry, not in `reviews[]`, so nothing surfaces them into a briefing automatically and each pass re-derives them from scratch. Write an explicit *already settled, do not re-derive* block: what was fixed and in which commit, what was **withdrawn** and why (an adjudicated item re-raised is worse than a missed one), and any third-party claim already verified. (Surfaced on #5828, whose prior round shipped six commits plus a summary comment; with the block in the briefings, none of the three passes re-raised any of it.)
...
1. **Concatenate** `findings[]`, `out_of_scope_observations[]`, `api_changes[]`, `leadStatus[]`, `callLog[]` across all three passes. Tag each `leadStatus[]` entry with its pass so a `could-not-verify` can be re-asked of the right focus, and check the union covers every lead you briefed — a lead briefed to one pass and absent from its output still needs answering (verify it yourself per step 7). Only Pass C's `api_changes[]` should be non-empty — Passes A and B emit `api_changes: []` per the focus contract in `code-reviewer.md`.
...
**The same applies when two passes disagree about *which function the code actually calls*.** Reachability is the case below, and it is not the only one: a pass can be wrong about the *identity* of the machinery — "the cache key compares constants via A" versus "…via B" — and that disagreement is decided by reading the call site, never by whose argument reads better. Treat it exactly like the reachability case: go to the source, name the function that actually runs, and report that. It is worth the call because such a disagreement usually decides whether a finding exists at all rather than just its severity. (Surfaced on #5841: one pass had the query cache comparing constants through `EqualsToVisitor.EqualsToX` — `Equals(value1, value2)`, reference equality for a collection — and another had it routing through `CompareConstant`, which is value-based. `Query.Compare` settled it in one read; the first pass was right, and that is what made a doc finding real, because the PR's new helper was documented as matching the comparison it was written to diverge from.)

**Two passes contradicting each other on *reachability* is a third answer, not a tie to break.** Dedup (step 2) handles two passes agreeing; nothing above covers one pass asserting a shape reaches the code and another asserting a guard blocks it. Neither is authoritative — each checked the route it happened to think of — so resolve it by reading the **guard** and enumerating what it actually excludes, rather than picking the pass with the better argument. The reason this matters more than a severity tie is that the surviving route is routinely one **neither** pass named: a guard written for case X often lets case Y through, and Y is the live one. Report the reconciled reach, not either pass's. (Surfaced on #5817: Pass A filed a DuckDB stand-in defect reachable "via a `RANGE`-offset frame"; Pass B showed `WindowFunctionsMemberTranslator` refuses a RANGE/GROUPS **offset** frame without exactly one ordering key, so that route is dead. Reading the guard showed it tests boundary type only — a non-offset `GROUPS` frame walks straight past it, which is the one route that *does* reach the code, and neither pass had it. A probe then showed DuckDB accepts the stand-in there anyway, so the finding was dropped: the mechanism was real, the consequence was not.)
...
**`leave` is for non-defects only — an out-of-scope *bug* gets filed.** When an observation is a genuine defect (wrong behaviour, or an exception on a supported combination), the expected disposition is **track**, not an FYI paragraph: a paragraph in a review body on someone else's PR is read once and lost, while a defect a reviewer has *already analysed* is cheap to file and expensive to rediscover. Reserve `leave` for things that aren't defects — a design asymmetry, a documented trade-off, a naming wart, an accessibility choice. **Probe before filing**, because a reading-derived claim gets details wrong in public: on #5816 the inferred `IndexOutOfRangeException` was actually `ArgumentOutOfRangeException`, and the repro described as exotic turned out to be two ordinary entity classes. When a related-but-distinct wart shares the same root cause, prefer a comment on the filed issue over a third issue. (Surfaced on #5809, where `leave` was recommended for two such observations — the reply was *"it sounds like a bug. no? if so - makes sense to fill it"*, and both became issues.)

**Verify each observation's factual claims against source before dispositioning it — and treat `PublicAPI.*.txt` completeness claims with the same suspicion.** These two categories fail verification far more often than line-level findings, for a structural reason: a line-level finding carries a verified anchor and a snippet, while these are narrative, so nothing forces the agent to check them — and they read as considered asides, which is exactly how they slip into a review body unexamined. Four of four such sub-claims were wrong on one #5750 round: a "DB2 diverges from the documented rule" claim where Oracle and PostgreSQL do the same thing, a "regression" that `origin/master` also exhibits, a `PublicAPI.Unshipped.txt` BOM complaint against a file matching 65 of 70 siblings, and two "missing from PublicAPI" members that live on an `internal` type. Two checks catch most of it: does the "diverging" code actually diverge, or do its **siblings** do the same; and is the type/member actually public, with the file state actually as claimed. `code-reviewer` OOS observations are narrative and have proved unreliable — a mechanism/behavior claim can be wrong, and an "already tracked by #N" reference can point at a discussion or a closed item rather than an open tracking issue. Before choosing promote / track / leave for an observation: (a) verify any mechanism/behavior claim against source at PR HEAD — read the cited file, don't trust the summary (same "files win over the reviewer's summary" discipline as the Baselines section); (b) for an "already tracked" claim, confirm #N is an open tracking *issue*, not a Q&A discussion or a closed item (`gh issue view <n>`). Drop or rewrite any observation the source contradicts. (Surfaced on PR #5600: an OOS "Oracle bulk copy bypasses CreateParameter" was false — the SQL-fallback path routes through it and the native path uses no `DbParameter`s; and "#5009 already tracks the bulk-copy gap" was false — #5009 is an unanswered discussion, not an issue.)
...
**An observation about an unexplained, wide-blast-radius change usually wants option 1 with a documentation ask, not option 3.** When the observation is "this change is deliberate and necessary, but nothing in the diff says why, and it widens what every query or every third-party extension point sees", the useful disposition is a `SUG` requesting a rationale comment at the site — not an FYI the author reads once and loses. Two tells that this applies: the *counterpart* half of the same change got a rationale and this one didn't, and a neighbouring line still carries the guard that was just removed here, so the asymmetry now reads as unexplained. Don't recommend skipping merely because the change is correct — correctness is why it stays, and the missing rationale is the finding. (Surfaced on #5750: `MemberTranslatorBase.Translate` dropped `{ Method: not null }` from its `BinaryExpression` arm — the whole change to that file, uncommented, while its counterpart in `ExpressionBuildVisitor` got four lines of rationale and the unary arm ten lines below kept its guard. Recommended as an FYI; the user's call was *"actually not skip but ask to add rationale comment"*.)
...
5. **Verify every identifier the suggestion introduces.** A `suggestion` block is one-click-applicable code, so an unresolved member ships broken code straight into the author's branch — and a suggestion that *looks* plausible is exactly what survives review. For each suggestion, whether you synthesized it or carried it over from a reviewer pass, confirm every non-local identifier it names (method, property, type, enum member) actually resolves in the cached HEAD tree, with the signature the suggestion assumes: `Grep` the declaring file under `<writeDir>/`, or the working tree for a file the PR doesn't touch. Do this **before** the manifest is built, not after posting. (Surfaced on #5750: BLK001's suggestion was carried over containing `QueryHelper.UnwrapNullablity(…).IsNullValue`; both members do exist — `QueryHelper.cs:1770` and an `ISqlExpression` member used across the tree — but that was confirmed only during the post-session reflection, after the review was already public.)

6. **Resolving is not enough — check the replacement doesn't discard behaviour the surrounding code depends on.** Identifier verification catches a suggestion that won't compile; it says nothing about one that compiles and is *wrong*. The dangerous shape is a swap to a neighbouring API whose name reads like a stricter/safer version of the original: it resolves, it applies in one click, and it silently drops a second concern the original call was carrying. Before shipping such a suggestion, name what the replaced expression supplies to its consumers and confirm the replacement supplies all of it. When two reviewer passes disagree about the fix, that disagreement **is** the signal — prefer prose describing both designs over a one-click block that silently picks one. (Surfaced on #5750: a pass supplied `QueryHelper.GetColumnDescriptorForTyping` in place of `GetColumnDescriptor` at `ContainsBuilder.cs:163`. It resolves and compiles — but the loose walk is also what carries a `[Duration]` column's unit and its value converter, so the "safer" call would have sent a raw `TimeSpan` to the driver. A parallel pass caught it; the suggestion was dropped and the comment named both options instead.)
...
4. **Cross-provider anomalies** under `### Cross-provider anomalies`, one bullet per entry.
   ...
   **A cluster in a domain the PR's feature doesn't touch is a repro candidate, not a note — and reading the baseline file does not discharge it.** The verify-against-the-file rule above answers *"did this change happen?"*; it cannot answer *"is the mechanism behind it lossy?"*, because the value in the baseline is whatever the test happened to use, and a benign-looking one hides a mechanism that damages a different value. When a cluster's test group has nothing to do with the PR's subject matter (a duration PR moving decimal literals, a paging PR moving collation), treat it as a lead: identify the mechanism in source, then **construct a value the mechanism would damage** and run it — against PR HEAD and against `master`, since the master control is what separates a regression from pre-existing behaviour. (Surfaced on #5750, on the third review of the same branch: the `SubQueryTests.Issue1601` ClickHouse cluster — `toDecimal128('8', 10)` becoming `toDecimal64('8', 4)` — was verified against the file and written up as a note in two earlier rounds, because `8` survives either spelling. It was a blocker: the literal takes its scale from a column descriptor recovered through `SUM`, so `0.00005` truncates to `0.0000` and the query answers a silently wrong number that `master` gets right.)

   **The converse also holds: an expression *absent* from a baseline is not evidence it was dropped.** The rule above covers a baseline that looks benign over a lossy mechanism; this is the mirror image — a baseline that looks broken over a correct one. linq2db routinely satisfies part of a projection **client-side** from a column the query already selects, so a term you expect in the SQL can be missing while the computed result is right. Both directions reduce to one rule: **a baseline establishes what SQL was emitted, never what the query computes, so severity requires a runtime probe with a value that discriminates** — one the correct behaviour and the suspected defect disagree about. Never assign a severity to a baseline-derived finding without that probe, and treat an incidental assertion that happens to pass as no evidence at all: it only discriminates if it was designed to. (Surfaced on #5783, twice in one review. First, all 53 baselines rendered `AllRows = Sql.Window.Count(w => w) == data.Length && Sql.Window.Sum(t.IntValue, w => w) > 0` as bare `SUM(...) OVER () > 0` with no `COUNT(*) OVER () = 9` anywhere; written up as a possible blocker for a dropped conjunct, it was disproved — an array-`.Length` access on a captured array is evaluated client-side off the already-projected `COUNT(*) OVER ()` column, and a `.Length` comparison with a deliberately wrong value duly returned `false`. A local `int` instead parameterizes and keeps the whole conjunction server-side. Second, in the same review `RowNumber(w => w.OrderBy(1))` was judged from a coverage test asserting only `RowNum >= 1` — which cannot tell a real ordering from a constant one — and was understated as "a provider-specific runtime error"; a probe built for that question showed a hard SQL Server error 5308 *and* a silent constant sort key on PostgreSQL, DuckDB and SQLite, filed as #5806.)
   ...
   **Scan the whole diff programmatically for structural signals — the grouped summary is sample-based and under-reports.** The subagent characterises 700+ files from sampled pattern-groups, so a change shape that spans groups it didn't sample is described narrowly or missed. **One call does the headline counts and scans 1, 2 and 4 below** — scan 3 is a short manual grouping over the same dump: `pwsh -NoProfile -File .claude/scripts/baselines-pr-scan.ps1 -Pr <n>` (pass `-Signal '<regex>'` when the PR's mechanism has a fingerprint other than a parameter `DECLARE`). Its `renames.mismatches` is the direct test of a "name-only, no structural change" claim — it normalises every parameter identifier on both sides of each changed line-pair and byte-compares, so `0` means nothing but identifiers moved, and `mismatchSamples[]` shows what did. Read the net-positive files it lists. Don't hand-roll these with `git diff` + throwaway analysis scripts. The scans, for reference:
   ...
   3. **`DECLARE` *type* changes over an unchanged body** (not covered by the script — group the `DECLARE` lines of the same `-U0` dump by their type token). The delta count in 1 catches a parameter becoming a literal — it says nothing about a parameter keeping its place and changing its **type**. Group the changed `DECLARE` lines by their type token and look for files whose `SET` / `SELECT` lines are byte-identical while `DECLARE @x DateTime2` becomes `DECLARE @x DateTime`. That is a parameter's *declared type* moving, which is a behaviour change the baseline's own value usually cannot show: the captured value is whatever the test happened to use, and any value both types represent — midnight, a small integer, a short string — hides the consequence completely. Never classify such a cluster as cosmetic on the strength of the diff; follow it to the mechanism and construct a value the narrower type cannot hold. (Surfaced on #5750, where it produced the review's most serious finding: 20 `TypesTests.DateTime1` files changed only that token, carrying `DATETIME2FROMPARTS(2009, 9, 20, 0, 0, 0, 0, 7)` — midnight, which `datetime` and `datetime2` both hold exactly. The mechanism was a descriptor walk lending a `datetime` column's type to a bound beside `CAST(x AS Date)`, and a pre-1753 bound threw *SqlDateTime overflow* at the driver where master answered. Neither of the three source-reading passes found it.)
   4. **Net-line split over the *modified* set, then read the net-positive files.** From the same `-U0` dump, count added vs removed lines per modified file and partition on the net. Net-zero files are almost always inert churn (the subquery projection's **column order** changed, or a config-name header rename); the net-positive ones are where new SQL appeared, and they are usually a small minority worth reading individually. Report the two totals separately, and never let a "cosmetic reorder" characterisation of the whole modified set stand until the net-positive subset has been read. **A synthetic `as [cN]` / `as "cN"` projection appearing on the added side is a specific signal to chase:** it means an expression the query previously only *inlined* is now materialised as an output column, which happens when a clause the optimizer used to discard now survives — check whether the parent query also gained a redundant trailing `ORDER BY` term referencing it. (Surfaced on PR #5732: the grouped summary classified the whole `DistinctByOrderByNulls` cluster as net-zero column reordering. The split showed 219 net-zero vs **83 net-positive**, and 75 of those had gained an `as [cN]` column — an emulated NULLS key both projected through the ROW_NUMBER subquery *and* appended as a duplicate of the outer `ORDER BY`'s leading key. That was the review's only real code finding, and the source-only pass could not see it.)

   (Surfaced on PR #5723: the grouped summary reported 17 files / 3 test groups "confined to WHERE-clause comparisons" and explicitly stated that SET-clause and INSERT-VALUES positions kept their parameters. A `DECLARE`-loss scan over the full diff found **47 files / 62 DECLAREs / 8 test groups**, including `InsertTests.Insert16`, `UpdateTests.Update14` and two `MergeTests` — so the shape claim was wrong as well as the count. The provider histogram in the same pass showed the modified set was confined to exactly the five `WrapParameters` providers with zero movement elsewhere, which became load-bearing evidence for two findings.)
   ...
   **A `changed_suspect` claim that implies the PR's own tests must be failing is almost always a superseded-run artifact — date the files before repeating it.** The baselines branch is **append-only across CI runs**: each run commits only what it captured, and baseline files are never pruned. So when an earlier run built a *different* version of the PR (a commit later reverted or force-replaced), the SQL it captured stays on the branch forever. At review time that reads as "provider X now emits real SQL for a test whose `[ThrowsForProvider]` says it must throw" — i.e. an apparent contradiction with green CI. Before surfacing it, date the files: `git -C ../linq2db.baselines log -1 --format="%cI %s" origin/baselines/pr_<n> -- '<path>'`, and compare against the branch's run boundaries (`git log --format="%H %cI %s" origin/master..origin/baselines/pr_<n>`, then look for a multi-hour gap separating runs). If the files were written by an earlier run **and** a later run re-ran the same provider legs without rewriting them, the tests are throwing exactly as declared and nothing is wrong. Report it as a superseded-run artifact, not a finding — and per `agent-rules.md` → *Push to remote rules*, do not turn it into a cleanup action item either.

   **Date it by the *author* date, and count the branch's commits first — `%cI` is the wrong field and `log -1 -- <path>` is the wrong probe.** CI's *Create Test Baselines Branch* phase recreates `baselines/pr_<n>` on top of current baselines master and **replays** the existing commits onto it, so every commit's **committer** date is the recreate time and tells you only when the latest run started. The **author** date survives the replay and identifies the run that actually captured the SQL: `git -C ../linq2db.baselines log -1 --format="%aI %cI %s" origin/baselines/pr_<n> -- '<path>'`. Equally, run `git rev-list --count origin/master..origin/baselines/pr_<n>` before dating anything — on a **single-commit** branch `log -1 -- <path>` returns that one commit for *every* path, so it cannot discriminate between files and a "the file is on the branch tip, therefore it is current" reading is circular. Cross-check the author date against each run's `Commit test baselines` task offsets (`azp-job-durations.ps1 -BuildId <n> -Offsets`): a run whose legs committed nothing wrote no baselines, which is itself the evidence that whatever the branch carries predates it. (Surfaced on #5828: the baselines pass reported *"I verified this is not a stale/superseded-run artifact — `git log -1` returns `13209a3fd1b`, dated 2026-08-29T14:44:22Z"* and concluded the current code still emitted the churn. Author date `08:44:21Z` placed the commit in the **previous** `test-all` run, two commits earlier; the fix under review had worked, and the three Oracle legs of the run that produced the `14:44` committer stamp had committed nothing at all.)

   (Surfaced on PR #5732: the baselines pass headlined that four `GroupByTests` methods × {Access ×2, MySQL 5.7 ×2, Firebird 2.5} "should currently be RED, inconsistent with the reported fully-green CI". All 14 files dated to the first run's 14:35–15:11 UTC window on a pre-revert commit, and the second run re-ran those exact legs without touching them.)
...
- On `interactive` (default): walk every reviewable item (body / line / file findings, out-of-scope observations, baselines anomalies, audited threads) per the orchestration doc's order, with per-item `fix | reject | accept-for-post`. When the item count exceeds 20, propose groupings first (per the orchestration doc). At the end of the walk, post the accumulated `accept-for-post` set as one draft review and run the same thread-disposition bundle. **Put the closing tally and the post/cancel question in the same turn** — a turn that ends with the final finding table and no question reads as "done" and stalls, costing the user a "and?" to restart it. The tally is the preamble to the question, not a turn of its own.
  **An empty `accept-for-post` set is a normal outcome — post nothing and say so.** A walk in which every item was fixed on the branch, tracked as its own issue, or dropped leaves no draft review to submit, and the wording above ("post the accumulated set as one draft review") reads as though something must go up. Don't manufacture a body to have something to post, and don't re-raise dropped items to fill it. Close with the disposition tally, the commits pushed, any issues filed, and one line stating that no review was posted because nothing was accepted for posting. (Surfaced on #5817: eleven items → six dropped, five fixed on the branch, one filed as its own issue, zero posted.)
