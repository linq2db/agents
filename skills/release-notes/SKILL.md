...
4. **Regenerate + diff.** `release-notes-draft.ps1 -Action apply-wiki -ManifestFile <path>`. The script requires a clean clone, ff-only-pulls, **rebuilds the entire `### Release <ver>` section** from the manifest (inherently idempotent), writes the page, and emits a git diff to `diffPath`. It does **not** commit or push.
   - **⚠ Commit between apply rounds, or a `git restore` silently reverts the previous one.** `apply-wiki` needs a clean clone, so a second round starts with `git restore` — which discards the *uncommitted* output of the first. If the section is then re-parsed to rebuild the manifest (the normal way to recover deep-dive bodies and untouched bullets), the older committed text is read back in and the first round's edits vanish from the manifest, hence from the regenerated page. The second diff looks clean because it is diffing against the reverted state. Either commit each round before starting the next, or hold the corrections in the manifest source rather than the working tree. (6.5.0: three naming fixes inside a deep-dive were applied, reverted, and only caught because the maintainer re-read the published page.)
   - **Verify the *written page*, not the manifest.** A check that greps `prBullets[]` misses **`deepDives[]` entirely** — they are separate arrays — so a rule applied only to bullets reports clean while the deep-dive still violates it. Run the final check over the regenerated `### Release <ver>` section on disk, which covers both by construction. (Same 6.5.0 incident: the bullets check returned 0 while three occurrences sat in the deep-dive.)
...
- **Crediting external contributors — who, and in what form.**
  - **Only implementation contributions are credited.** The contributor must have provided the fix or feature — a commit of their own — **even if it was discarded and rewritten**. A **plain test contribution is not advertised**, and neither is a bug report.
  - **Never write a "Reported by …" clause at all.** Reporter attribution already lives on the linked issue; repeating it in the notes is noise. (6.5.0: removed from #5780, #5801, #5802, #5809.)
  - **Form: always a profile link — only the link *text* varies.** The contributor's **name** when the profile has one (`Thanks to [Ilya Chudin](https://github.com/ili)`), otherwise the **`@handle`**, still linked (`Thanks to [@cal-tlabwest](https://github.com/cal-tlabwest)`). Never a bare unlinked `@handle`, and never shorten a handle into a pseudo-name — `Thanks to [cal](…/cal-tlabwest)` is wrong twice over. Check with `gh api users/<login> --jq '{login,name}'`; a `null` name means use the `@handle` form.
  - **Establish the contribution type before crediting**, because the PR author is often a maintainer who applied someone else's work: `gh pr view <n> --json author,commits --jq '{author: .author.login, commitAuthors: [.commits[].authors[].login] | unique}'`. On 6.5.0 this separated #5813 (cal-tlabwest authored the implementation commit → credited) from #5809 (no commits → not credited).
- **Product naming: `LinqToDB` (or `Linq To DB`) in prose; `linq2db` only as a file or package identifier.** `linq2db` is the NuGet/file name, not the product's name, so it belongs in `` `linq2db.cli` ``, `` `linq2db.Analyzers` ``, the core `` `linq2db` `` package and URLs — never in a sentence. Write "LinqToDB failed on it with an `InvalidCastException`", not "linq2db failed on it". (6.5.0: nine occurrences, incl. three inside a deep-dive.)
- **Never use a test-side or internal term the user cannot see.** Method-name variants that exist only to bracket provider versions in the test matrix — `2012Plus`, `2014Plus` and friends — tell a reader nothing; name the API family instead. This is the same rule as the internals ban above, applied to identifiers rather than mechanism.
- **Every bullet must stand alone — no dangling references to other bullets.** Bullets are regrouped under component and change-type headings, so neighbours at drafting time end up far apart or in different sections. "**the same** lost-brackets defect", "so **this** applies to every call", "as **above**" all read as continuations of text that is not there. State the thing in full in each bullet, even at the cost of repeating a clause. (6.5.0 #5750 ClickHouse and #5828 Oracle.)
- **Don't repeat the component name inside its own bullet.** Under `#### Informix`, a bullet starting "Informix: …" or "**Informix** — …" is redundant. If a draft groups provider bullets under a `**Provider bullets**` header, strip that prefix when placing them — and **re-capitalise the first letter**, since removing it leaves the sentence starting lowercase.
- **Keep emphasis consistent within a sentence.** If two items in a list are bold, all of them are: "the **gRPC** and **WCF** transports … the HTTP and SignalR clients" gives two of four names weight for no reason. Prefer no emphasis unless it earns its place.
- **No historical asides.** Where a magic number came from, which ancient version a limit was written for, what the constant used to mean — none of it changes what the reader does. "The old value came from Oracle 8 documentation" is provenance for the commit message, not the notes.
