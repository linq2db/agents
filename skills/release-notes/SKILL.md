...
- **Crediting external contributors — who, and in what form.**
  ...
  - **Form:** the contributor's **name** as the profile link text when the profile has one (`Thanks to [Ilya Chudin](https://github.com/ili)`), otherwise the bare **`@handle`** with no link (`Thanks to @cal-tlabwest.`). Never shorten a handle into a pseudo-name — `Thanks to [cal](…/cal-tlabwest)` is wrong twice over. Check with `gh api users/<login> --jq '{login,name}'`; a `null` name means use `@handle`.
