...
**Name the tree the scouts must search, in every prompt — the branch base, not wherever you happen to be.** [`agent-rules.md`](../../docs/agent-rules.md) → *A working-tree search is evidence about your checkout* is the rule; the scout dispatch is where it actually has to be applied, because a subagent searches whatever path it is given and reports a confident, well-cited answer about the wrong tree. The primary clone is routinely dozens of commits behind `origin/master`, so pass the **worktree path** (or an explicit ref for `git grep`) and say so in the prompt. Run `git -C <clone> rev-list --count HEAD..origin/master` first if you are unsure which tree you are about to hand them. The failure is not partial: it does not produce one stale row, it produces a map that asserts absent files exist and present ones do not, and a critic that re-derives against the real base refutes the whole block. (Surfaced 2026-09-03 on `issue/5731-third-party-license-notices`: four scouts were pointed at a clone 49 commits behind, and the resulting `P7` invented an absent `.github/workflows/` and a `.lpx6` artifact the repo had stopped producing. Round 1 came back `refuted` for that reason alone. `agent-rules.md` already carried the rule — it just never reached the scouts, which is `GAP-07` by the ledger's own taxonomy.)
...
**When the sweep is for *contradictory* conditions, also sweep for *repeated* ones — a predicate can be wrong against itself.** A search built to find two predicates that cannot both hold is written as a pair of *different* tokens (`A.*&&.*B`), and that shape structurally cannot match the case where the same predicate appears twice: a guard duplicated further down the same method, or one disjunct repeated inside a single `||` chain. Both are defects of exactly the kind the sweep exists to find, and both read as ordinary code. Add a same-token pass — the identical predicate twice in one condition, and the same top-level guard twice in one method body — or say plainly that the map covers contradictions only. (Surfaced on #5814: the plan's `&&` sweep and the critic's wider `&&`/`||` sweep independently produced a 10-row impact map, and the analyzer's first run over the real assembly found an eleventh — `!flags.IsSql()` guarding an early return and then re-tested 24 lines later in the same method — plus `flags.IsExpand()` written twice in one `||` chain that neither sweep could express.)
...
**When a `P5` decision *admits* a symbol kind, input class or node type into a detector's scope, the plan owes a `P4` row per admitted kind — probed through *every* consumer, not just the detector.** Admitting something is a deliberate act, which is what makes it feel already-considered; but the decision block reasons about whether the kind *belongs* in scope, and the defects live in what each downstream stage does with it once it is. Send a scout per admitted kind, and have it walk the detector, the code fix, and any message or name rendered from the symbol. The worked example is `MethodKind.ExplicitInterfaceImplementation`, admitted on #5870 and never probed: its `ISymbol.Name` is the dotted `I.M`, which silently killed a name-equality walk, and its bare identifier does not compile inside `nameof`, which made the code fix emit `CS0103`. The neighbouring kind that *did* get a `P4` row — property accessors, and their `OwningSymbol` → `AssociatedSymbol` hop — shipped correct.

**When the edit surface is "a set of sites selected by a pattern", enumerate by the site's *semantic* key, not by the text that names it — and by where each site routes.** A registration, a mapping-table entry, an attribute usage: the natural sweep is a regex over the declaring text, and it is wrong in two independent ways. It matches the *wrong half of the line* — `Registration.Register.*Sql\.` counted every registration whose **translator lambda** mentioned `Sql.DateParts`, reporting 53 `Sql.*` registrations in one file where the real count, by registered member, was 11. And it cannot see **routing** — whether two registrations sharing a declaring type dispatch to the same delegate, which was the property that actually decided whether each could be opted in. Two registrations of `string.Concat` differing only in arity went to different delegates, one of which reached `AggregateFunctionBuilder` and had no client-side equivalent at all. Require the scout to report the count **by the key the change turns on** (the registered member, the mapped symbol) and to name, per group, which delegate or handler the sites reach; a count produced by a line grep is a hypothesis and must be labelled one. (2026-09-14, `feature/optional-member-translations`: the critic refuted the plan on this class twice, and the third round still found a missed aggregate-routing overload.)

**When the edit surface is keyed by a registered member, also sweep what *lands on* each key before the gate — and settle a `P4` census across every `E-n` it bears on.** The semantic-key rule above enumerates the registrations; it does not see the calls that reach them without naming them. A pre-gate rewrite — a `MapMember` entry in `Linq/Expressions.cs` (the common table and the per-provider ones), an `[ExpressionMethod]` body, a `Sql.ConvertTo`-style expansion — replaces the user's call before the registry is consulted. Every member rewritten *into* a key inherits whatever the change does to that key, and anything evaluated afterwards is the expansion, not the method the user wrote. Have the scout list, per key, the rewrites that target it and whether each expansion is semantically the same method. Separately, a `P4` question answered by a census ("can an opted-in registration throw client-side?") is answered only for the files the census searched: resolve it for every `E-n` it bears on, or narrow the row's claim to the files it covered. (2026-09-14, `feature/optional-member-translations`: `string.Compare` / `CompareOrdinal` are rewritten into `s1.CompareTo(s2)`, so opting `CompareTo(string)` in made the client evaluate a culture-sensitive comparison where an ordinal one was written; and `U-11`'s throws-census covered `StringMemberTranslatorBase` only, while `E-5` opted in 34 `Convert.ToXxx` overloads .NET documents as always throwing.)

**When the defect is "this site receives an input it cannot handle", enumerate the input's *producers*, not just the consuming sites.** A site-shaped sweep is the natural one — grep the function name, get every call — and it is *complete* and still yields a `P1` that describes the wrong population, because it answers "where is this emitted?" and never "where do its arguments come from?". The tell is a problem statement whose reach is stated in terms of what the **user** declared: if the bug is framed as "happens when the user writes X", ask which of the inputs reaching the site are manufactured by *our own* code, and what they look like there. Those are the ones nobody reports, because nothing in the user's source points at them. Send a scout per input position with the question *"what constructs the value that lands here, and what is its type at the boundary?"*, and have it report the producer's declared type **beside** what the downstream system actually sees — a mismatch between those two is both the bug's real extent and the reason a type test cannot fix it. (Surfaced on [#5955](https://github.com/linq2db/linq2db/issues/5955): scouts enumerated all six emission sites of a ClickHouse function family correctly, and `P1` still said the bug needed a user-declared column type. Three of the four operand kinds that reach those functions are built by the provider's own translator — `now()`, `makeDateTime(...)`, and the `toDate32` a `.Date` truncation emits — each typed `DateTime64(7)` internally while the server types it `DateTime` or `Date32`, so the regression had been breaking default-mapped columns all along. The critic found it; two obligations and the whole framing of the fix changed.)

**A test obligation that depends on a harness capability must name that capability as a `P4` unknown.** `P8` is written in terms of what will be *asserted*, which hides the question of whether the harness can set the run up at all. The bulk gate on the same branch was "run the suite with the option forced on globally" — and no such hook existed; it surfaced at execution time and needed a temporary, uncommittable edit to `TestBase`. Ask of every obligation: what has to be true of the runner for this to be executable? Then resolve it as an unknown before approval, not during.
...
- **A `red→green` label is a claim about the current corpus — verify it before approval, don't inherit it.** The label promises the obligation fails against unfixed code, and the way it goes wrong is not a test that fails for the wrong reason (vector 3 of the critic's list) but a test that is **already green**. It happens when tests are being ported or re-pointed and the author takes "coverage was never written" from a prior document — an earlier plan, an issue, a previous session's note — rather than from the fixture. Grep the target fixture for the API the obligation exercises: if it is already called there, the obligation is `characterization` and labelling it `red→green` claims proof the plan will not get. Evidence for a `red→green` row is a `file:line` showing the gap, never a plan reference. (Surfaced 2026-09-20 on the #1475 removal plan: two of six ported scenarios were labelled `red→green` on the strength of an earlier plan listing them as unwritten obligations; the fixture already composed `.Where` over a generated cell at two call sites and already exercised `Max`, so both were green against the unchanged branch.)
