...
- **Bisect-across-SDK gotcha**: `.claude/docs/windows-dev-gotchas.md` → *Bisecting across SDK upgrades*
...
3. **Pick the bisect providers.** Pass `--provider` — typically `SQLite.MS` plus one provider matching the test's domain — rather than editing any `Providers` array: it replaces the active set per run, and the connection strings come from the tracked `DataProviders.json`, so the worktree needs no `UserDataProviders.json` for this. **A bisect range reaching back before 2026-06-17 crosses the commit that added the flag** (`3c345be327`, PR #5621): on candidates older than that, `--provider` is an unrecognised argument and provider scoping reverts to a trimmed `UserDataProviders.json` at the worktree root, with the pre-MTP runner invocation to match ([`testing.md`](../../docs/testing.md) → *Scoping a run to specific providers*). Check the range against that date before scripting the loop — the method has to switch mid-bisect, not the whole run fail at one end. Copy the main repo's in only when the bisect actually needs what that file adds — a local connection-string override, or `BaselinesPath` for a baseline-sensitive test. Note the sibling path suggested above is **non-nested**, so nothing is inherited from the primary clone; a nested worktree resolves the clone's file on its own (`worktree.md` → *`UserDataProviders.json` in a worktree*). Containers for the chosen providers must already be running in the main session — the skill doesn't touch docker.
4. **Loop.** For each candidate commit:
   ...
   - `dotnet test --project <project> --filter "FullyQualifiedName~CreateData.CreateDatabase|FullyQualifiedName~<TestClass>.<TestMethod>" -c Debug --settings .runsettings --test-progress -p:TreatWarningsAsErrors=false -p:NoWarn=CS9336` — the `TreatWarningsAsErrors=false` + `NoWarn` flags are needed when bisecting across SDK upgrades (see [`windows-dev-gotchas.md`](../../docs/windows-dev-gotchas.md) → *Bisecting across SDK upgrades*); extend `NoWarn` with extra IDs as historic-code warnings surface.
