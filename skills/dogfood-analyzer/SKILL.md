...
**And when the rule's branch touches no file under the consumer project, an existing worktree on that branch *is* the pristine corpus — reuse it instead of checking out a second one.** The reason to base on `origin/master` is that the corpus must be the one the prediction was made against, not that the worktree must be fresh; a branch whose diff is confined to `Source/LinqToDB.Analyzers` and `Tests/Tests.Analyzers` has a `Tests/Linq` tree identical to master's, and confirming that from the PR's own changed-file list is cheaper than a full checkout. Two conditions, both worth stating rather than assuming: the branch has merged master (else its `Tests/Linq` is merely *old*, not pristine), and the scratch edits — the consumer `.editorconfig` and the `<Analyzer Include>` — are reverted before anything is committed, since here they land in a worktree you *are* committing from rather than a throwaway. Re-running the dogfood after a review round is exactly this case: the analyzer under measurement is the fixed one, which a `origin/master`-based worktree cannot see. (Used on #5873's second review round, after five behaviour changes: same seven sites.)
