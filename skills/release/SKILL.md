...
1. Determine state-file path: `.build/.agents/release-<version>.json`.
...
3. On "yes":
   ...
   - Once user reports the merge done (or skill detects `state=MERGED` on next probe), update state: `currentPhase = "publish"`, write `.build/.agents/release-<ver>.json`, transition to `Skill('release-publish')`.
