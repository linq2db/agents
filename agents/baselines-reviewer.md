...
3. **Read `sizeOutliers[]` and `regressionCandidates[]` before pattern grouping.** These two surface tests whose shape regressed in ways the dominant-pattern view hides:
   ...
   - `regressionCandidates[]` — patterns where the heuristic archetype scan flagged a suspect shape on the `+` side that doesn't appear on `-`. Archetypes:
     ...
     - `expression-replaced-by-operands` (a `CASE`/`IIF` on the `-` side replaced by the bare columns it chose between — see *shorter SQL is not a smaller finding* below)

   **Shorter SQL is not a smaller finding.** Every other archetype detects SQL getting *more* complex, so a diff that gets *simpler* reads as an improvement and gets waved through. `expression-replaced-by-operands` is the one that goes the other way, and it is never cosmetic: the database stopped computing something, which means the client now does. In a projection that costs columns; under a set operation `DISTINCT` dedupes the operands instead of the value that was chosen, so **the row count changes**; in a predicate there is no client-side fallback at all and the query stops translating. Classify one of these as `changed_expected` only with a note saying what now evaluates client-side and why that is harmless *for each* of those three positions. A genuine optimizer simplification fires here too — that is a correct hit worth a sentence, not a false positive. (Surfaced on #5750, where `ValueConversionTests.ConditionUnionTest` turned `IIF(EnumNullable IS NOT NULL, EnumNullable, Enum)` into the three columns it chose between across 68 providers. The delta was 8–14 lines, far below `sizeOutliers[]`' top-20 cut across 7185 changed files, and no archetype fired — the maintainer caught it by eye. Regression tests then showed a sibling `Union` returning ten rows where three were selected, and a sibling `Where` failing to translate outright.)
...
4. For each logical group in `testGroups`, compare the per-provider diff bodies in `sql[]`. Classify into one of four buckets:
   ...
   **Order-dependent output is a regression, not drift.** When a baseline moves because a value derived from *execution order* changed — a process-wide counter reaching a generated name (`TestUtils.GetNext()`), a shared fixture cache captured by whichever provider populated it first, a field that accumulates across calls — classify it `changed_suspect` naming the order-dependent source. Do **not** wave it through as `changed_expected` on the grounds that the value is incidental and the SQL still correct. Both things are true and neither is the point: a baseline that moves with execution order produces churn on every unrelated PR and trains reviewers to skim past the file that is supposed to be the regression net. The tells are diffs confined to a numeric suffix, a run of whitespace, or the same block migrating between two provider variants, with no structural SQL change — exactly the shapes that read as benign. This applies with full force to a test-infrastructure PR, which by definition should not move SQL at all: there, an order-dependent move is the *only* kind of finding available and must not be reported as "expected drift". (Surfaced on #5614, where whitespace padding from an accumulating fixture field (49 files), a `GetNext()` suffix (62 files) and a first-writer-wins Northwind preload (4 files) were all classified as benign; the maintainer's call was *"all others look like regressions at least for baselines/baselines stability"*, and each one became a fix.)
   **A moved baseline says *that* the SQL changed, never *which side was right*.** Before classifying a provider's movement as an improvement — an "incidental fix", "now consistent with the others", "the transform finally reaches it" — establish whether the **old** SQL was valid *on that provider*. The direction of the change is not evidence: a fold/cast/wrapper exists because the bare form is invalid *somewhere*, and it is easy to conclude the provider that just started receiving it must previously have been broken. Often it wasn't, and the evidence is already in the repo: a green, value-asserting test on `master` for that exact baseline proves the old form worked, in which case the movement is redundant SQL rather than a repair. Providers whose boolean is an integer (ClickHouse `Bool` = `UInt8`, MySQL, SQLite) are the recurring case, since they accept aggregates and operands that the transform assumes need rewriting. Report it as *"provider X now emits <shape> where it previously emitted <shape>; results identical"* and leave the fix-or-regression call to the walk. (Surfaced on #5725: `GroupByTests.Max11`/`Max12` moved from `maxOrNull(t1.ChildID > 20)` to `maxOrNull(CASE WHEN … THEN 1 ELSE 0 END)` on both ClickHouse configs, and was written up as an incidental fix of a ClickHouse-specific gap. The maintainer's reply was *"you want to say sql regressed for ch?"* — `master`'s form was valid and its two value-asserting tests were green, so the real change was a redundant `CASE`, which became a provider opt-out commit.)
...
- **Don't put paraphrase, annotation, or a cross-reference inside `sampleDiff`.** That field is defined as the *raw* diff body, so the parent skill renders it in a ```diff fence and a reader quotes it as file content. The rule directly above means **elide** — truncate and set `sampleDiffTruncated`, or drop the entry to a summary line — it does not license substituting invented text shaped like a diff. Whatever you put in that field must be bytes that exist in the file. (Surfaced on #5801: entries came back carrying `-- (main query, no filter)`, `... WHERE [d].[Value] IN ('A')` and `(same 4-query body as LoadWithOptionalFilter(False))` inside `sampleDiff`. The substantive conclusions were right and held up when checked against the files, but none of those lines appear in any baseline — a review body quoting them would have published SQL the product never emitted.)
