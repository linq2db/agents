...
- **Always pass `--test-progress`.** It enables the live heartbeat at `.build/.agents/test-progress.<tfm>.<pid>.json` (see [Tests/Base/TestProgressReporter.cs](../../Tests/Base/TestProgressReporter.cs)), which `/test` and the user poll via `/test-progress` / `test-status.ps1`. It's a plain CLI option (no env var); a bare `--test-progress` uses the default path.
...
- **Run `dotnet test` to completion within your turn — never launch it in the background and return early.** If the overall run is long, the *caller* manages backgrounding at the Agent level; your job is to block on the `dotnet test` process and report real parsed pass/fail counts. **Being invoked with `run_in_background: true` is not licence to background the inner `dotnet test`** — that is the caller already handling the long-run case, and it makes your turn the thing that must block. Returning while a process you launched is still alive is a contract violation, not a status update: the caller cannot distinguish it from a run that never started, and has to verify liveness and poll the heartbeat itself. (Violated five times in one 2026-08-12 session, including once reporting `"exited normally, exit code 0"` for a process still burning CPU, and once citing an unrelated run's artifact as proof.) Ending the turn while the run is still executing (returning "still running / awaiting completion" with no counts) forces the caller to resume you via SendMessage and wastes a round-trip. When judging progress, note the heartbeat at `.build/.agents/test-progress.*.json` can belong to a prior / full-suite run — a filtered run of a handful of tests showing an `N/10595`-style total is the tell; verify the file's pid / recency before trusting it as this run's progress.
- **A `Tests/Linq/Tests.csproj` run cannot finish inside one Bash call — block up front instead of starting it.** The Bash tool's hard ceiling is 600 s and a cold `Tests/Linq` build alone exceeds that, so the rule above (block until completion) is unsatisfiable for that project: the tool involuntarily backgrounds the process mid-build and you end the turn with no output, having burned the full ten minutes. Being invoked with `run_in_background: true` does **not** help — that backgrounds *your* turn, not the inner Bash call, and its cap is unchanged. So when `project` is `Tests/Linq/Tests.csproj` and the build is not already warm, return `blocked` immediately, naming the two things the caller can do that you cannot: run it themselves via the PowerShell tool as a detached process, or re-scope to `Tests/Tests.Playground` (which builds fast enough to fit, and is the default per *Default to Playground + net10.0* above). Reporting this in the first minute is worth far more than discovering it in the tenth. (Cost two ~15-minute blocked returns in one 2026-08-21 session, on the same filtered single-test run.)
