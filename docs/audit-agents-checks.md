...
- **Recognized short name.** `model:` value not in `{opus, sonnet, haiku, fable}` → error. Specific full IDs like `claude-sonnet-4-6` pin to a point release that ages out; suggest the short name in the proposed fix unless the user explicitly indicates they want version-pinning.
- **Model fit** (warning, `fixKind: creative`). Convention: `opus` for heavy synthesis (`code-reviewer`), `sonnet` as default for moderate analysis (`baselines-reviewer`, `test-writer`, `kb-research`, `kb-architect`, `kb-historian`, `kb-issue-detector`), `haiku` for narrow execution (`test-runner`, `kb-github-curator`), `fable` **only** for an independent adversarial channel whose value depends on *not* sharing the author's priors (`plan-critic`). Flag when the assignment looks off — e.g. a review / audit / synthesis subagent (description contains "review", "audit", "judgment", "compare") pinned to `haiku`, or a pure-execution subagent (description is dominated by "run", "fetch", "execute" without a synthesis step) pinned to `opus`. Flag `fable` on an authoring or breadth agent: the point of that pin is the model *difference*, not extra capability, so it earns nothing outside a cross-model critique. The user owns the call; surface as creative so they can keep, change, or override.
- **Cross-model pins are host-dependent.** `plan-critic`'s effective model is chosen per host tool and stored in `.claude/plans/config.json`; its frontmatter value is the Claude Code default, not a universal one. Don't flag a mismatch between that file and the frontmatter — the skill passes the model explicitly on dispatch, which is what actually applies. That file's per-tool value is also allowed to be `never` or `ask` instead of a model id (see [`work-plan.md`](work-plan.md) → *Settings*), so a config carrying no model pin is a user preference, not a defect — and it is gitignored, so on most clones it is absent entirely.
