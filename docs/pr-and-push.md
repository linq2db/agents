...
### Syncing a branch: the base is not always `master`

**Read the PR's actual base before any sync merge** — `gh pr view <n> --json baseRefName`. Since GitHub shipped stacked PRs this repo uses them for some features, so a feature branch is routinely based on *another PR's branch* rather than on `master` (#5708 was based on `issue/5948-gate-postgresql-filter`, the branch of its dependency #5949). Merging `master` into a stacked branch does not clear a conflict against its real base and can create one, because master carries commits the base branch does not. Take "sync with master" as meaning the **base** branch whenever the PR has one — and note that a stacked PR cannot merge before its base PR does, so the merge order is a real constraint, not a formality.

The usual trigger for the conflict is the base branch being **rebased** by its own author. `git fetch` reports a forced update (`+ 4ad776d1c...e85eb6278 … (forced update)`), and the branch then holds the old copy of the base's commits while the base offers the same patches under new hashes; GitHub flips the PR to `CONFLICTING` / `DIRTY`. Merging the base resolves it, and the merge normally carries **no code change at all** — the two copies are the same patch, so the only conflicts are in files both lineages touch. Confirm that rather than assuming it: `git diff HEAD --stat` before committing the merge should be empty or near-empty, and anything else in it deserves an explanation.
...
### Immediately after `gh pr create`: check the diff shape

**Check a new PR's diff shape immediately after `gh pr create`, before writing the body.** `commits` count and the file list, one `gh pr view <n> --json commits,files` call. A branch created off *another branch* that is destined for squash-merge shares no commit with master once that merge lands, so GitHub computes the diff against the merge base and re-adds every file the parent branch introduced: a 40-line change was published as **373 additions across 9 commits**, unreviewable. The content would have merged correctly, which is why nothing else catches it. This happened three times in one session before the check became habitual — the fix is cheap (rebuild the branch on master, one commit) and the detection is one call. Related: [`pr-and-push.md`](pr-and-push.md) → *After every successful push*.
