...
>
> **Populating the submodule does not fully repair the session that found it empty — restart the agent.** The two halves behave differently, and only one of them is load-once:
>
> - **The always-loaded instruction set** (the root `CLAUDE.md` trampoline's imports — `AGENTS.md`, `.claude/CLAUDE.md`, `agent-rules.md`) is resolved **once, at session start**, and is *not* re-read when `.claude/` appears. A session that started against an empty corpus keeps running with no project rules however many files land on disk afterwards. This is the load-bearing half: it reads as "the corpus is broken" rather than "restart".
> - **Skill discovery can refresh mid-session** — measured 2026-07-31 in the session that performed the migration: right after `git submodule update --init`, the harness re-issued its skill list from the newly-populated corpus (a deleted skill vanished, reworded descriptions updated) while the always-loaded set stayed on the pre-migration copy. But it is not guaranteed — the reviewer of linq2db#5735 saw a freshly-initialized submodule with all 34 skills present still report `kb-status` as an unknown skill.
>
> So don't reason from either behaviour: restart is the only way to be sure both halves match what's on disk.
