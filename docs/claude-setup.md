...
>
> **Populating the submodule does not repair the session that found it empty — restart the agent.** Skills, subagents and the `CLAUDE.md` import set are resolved once at session start, so a session opened against an empty `.claude/` keeps resolving no skills and no project instructions after `git submodule update --init` succeeds. Every corpus file is on disk and nothing in the session reflects it, which reads as "the corpus is broken" rather than "restart". This is the load-once behaviour the *Harness mechanics* section below relies on; the same applies to a corpus file added mid-session. (Surfaced in review of linq2db#5735, where a freshly-initialized submodule with all 34 skills present still reported `kb-status` as an unknown skill.)
