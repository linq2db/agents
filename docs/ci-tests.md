...
**The table above runs one way, so don't read it as "always answer `test-all`".** Every row is *agent proposed scoped → maintainer said all*, which makes `test-all` look like the safe default that is never wrong. It is not: the correction has landed in the other direction too. 2026-09-06, [#5885](https://github.com/linq2db/linq2db/pull/5885) touched `Directory.Packages.props`, `nuget.config`, `Source/LinqToDB.EntityFrameworkCore/` **and** `Tests/Base/TestConfiguration.cs` — by the mechanical rule an unambiguous `test-all`, and that is what was proposed — and the instruction was *"run test-mysql on ci"*. The distinguishing feature is that the PR's whole purpose was **turning one provider on for one TFM**: every other leg was byte-identically unaffected (the other TFMs keep their existing package, the library edit only adds a `switch` arm), so `test-all` would have spent the full matrix to learn one thing. The generalisable shape is *a change whose observable effect is confined to enabling or disabling one provider*, which is narrower than "a change that touches provider-specific files". Still offer rather than assume — one counter-example against four does not flip the default.
...
**Waiting on a *GitHub Actions* run has a blocking form — use it instead of re-checking `gh pr checks`.** `gh run watch <run-id> --exit-status` blocks until the run finishes and exits non-zero if it failed, so launching it with `run_in_background` converts a CI wait into exactly the notification shape the rule above assumes; polling `gh pr checks` in a loop is the same waste as re-`Read`ing an output file. Two companions worth knowing: `gh run rerun <run-id> --failed` re-runs only the failed jobs and leaves the others' green in place (useful against a flaky leg — it also *distinguishes* a flake from a real failure in one cheap run), and `gh run view --job <job-id> --log-failed` prints only the failed steps, which is what you want instead of `--log` on a job that produced tens of thousands of lines. Get the run id with `gh run list --branch <branch> --limit 1 --json databaseId` — **except for a run started by an `/azp run` comment, which that query cannot see**: `tests-comment.yml` calls the workflow on `refs/pull/<n>/merge`, so the run is listed under neither the branch nor `tests.yml`'s own dispatch history, and the query answers with the branch's unrelated `build` run instead of nothing. Take the id from the PR check instead — `gh pr view <n> --json statusCheckRollup`, then the `targetUrl` of the `tests [<surface>]` context. (#5962, two wasted calls.) (Used throughout the #5731 CI rounds on 2026-09-03; three consecutive red→green cycles were driven this way with no polling.)

**Two limits on that blocking form, both of which bite on a long run.** It only converts a wait into a notification while the run fits the Bash tool's **600 s ceiling** — past that the task is killed and reported as *"failed with exit code 1"*, which is indistinguishable from the run itself failing, so the wait silently becomes a false red. And its output file is the **entire job tree re-printed every interval**, so `Read`ing it costs O(intervals × tree): one such read returned 986 lines / ~25 k tokens of the same tree repeated. For anything longer than ~10 minutes — a provider `test-all` is hours — skip the watch and check `gh api repos/<o>/<r>/actions/runs/<id>/jobs --jq '[.jobs[] | .conclusion // .status] | group_by(.) | map({state: .[0], count: length})'` on a wide cadence, then pull the non-green ones by name. For the recurring case — *is this PR's DB-free gate green yet* — [`wait-pr-checks.ps1`](../scripts/wait-pr-checks.ps1) does that wide-cadence poll for you and returns one summary plus an exit code (0 green / 1 non-green / 2 timed out), gating on the six GH legs and treating the Azure contexts as informational (see [`github-actions.md`](github-actions.md) → *`build.yml` is a superset of Azure's `build` pipeline*). **`-Gate` overrides that list, which is how you wait on a single provider leg**: an offloaded leg surfaces as one commit status named `tests [<surface>]`, so `-Gate @('tests [clickhouse.all]')` waits on the ClickHouse leg and nothing else (#5962, a 75-minute wait that returned one line). **Azure DevOps has no blocking form at all**; use [`azp-wait.ps1`](../scripts/azp-wait.ps1), which polls to a terminal state and returns the job list in one call, rather than re-calling `azp-build-info.ps1` (ten such polls on #5880, every one asking whether it had started yet — its queue wait was 7h17m). (Surfaced on #5880.)

**`azp-wait.ps1 -UntilJobsAppear` is for a build whose timeline is *empty*, and returns instantly otherwise.** It exits as soon as **any** `Job` record exists — and on the `default` definition a `Build` job is in the timeline from queue time, so the condition is already true on the first poll. The call then exits 0 within seconds, reporting the build as still `inProgress`, which reads exactly like a wait that ran and completed. Reserve the switch for the case it was written for (a queued build with no jobs yet); when you want *"tell me when stage X is done"*, poll for a **named job reaching a terminal state** instead. Terminal-state mode is the other wrong end — it waits for the whole matrix, long past the point the thing you cared about finished. (6.5.0: one wasted wait on build 23573.)

**The `default` definition on `refs/heads/release` is not the PR build — it runs no provider matrix.** A release-branch build is just `Build` + `Nugets Generation`: **38 minutes** end to end on 6.5.0 (`Build` 33 min, publish job 4 min), against ~18 hours of wall-clock for the same definition's PR build with its 32 provider legs. So don't estimate the post-merge publish wait from the PR run — and don't wait for the *build* to finish before checking the packages, since `Nugets Generation` starts the moment `Build` ends (on 23573: `Build` ended 14:55:11, publish job started 14:55:26, nuget.org pushes done by 14:59).

**`wait-pr-checks.ps1` exiting 0 does not mean the PR can merge.** Its "Azure contexts are informational" rule is a claim about *coverage* — `build.yml` really is a superset of Azure's `build` pipeline — but branch protection does not read it that way: `master`'s `required_status_checks.contexts` is exactly `["build"]`, the Azure one. So the script can report *all gated legs green* while `gh pr view --json mergeStateStatus` still says `BLOCKED`, which reads as a puzzling contradiction if you take exit 0 as a merge signal. Treat exit 0 as "the DB-free gate is in", then check the PR's own merge state, and use [`azp-wait.ps1`](../scripts/azp-wait.ps1) for the Azure leg that actually gates. `gh api repos/linq2db/linq2db/branches/master/protection` is the one call that settles what is required — it also carries the review requirement, which is the *other* half of a `BLOCKED` a green build cannot clear. (6.5.0 prep PR #5905.)
...
**Check `.status` / `.result` before trusting a build's failures at all — a *cancelled* build reports its killed tasks as failures.** `azp-build-failures.ps1` is faithful to the timeline, and a cancelled build's timeline is full of tasks that were terminated mid-run, so it returns a plausible-looking `failures[]` describing nothing real. On 2026-09-05 build 23400 (`completed/canceled`) yielded four failures including a ClickHouse connection error, and the conclusion "Azure fails this test too" was built on it and stated twice before the cancellation was noticed — the valid build for that PR, 23407, had two entirely different failures and no ClickHouse one. A cancelled build's `/timeline` endpoint also starts returning **HTTP 204 No Content** once it is cleaned up, so the same script that worked minutes earlier suddenly reports nothing; read that as "this build is gone", not "this build is clean".

**A PR check's `detailsUrl` names *which build* it came from, and the checks list shows the newest.** When a PR has had more than one run of the same definition, `gh pr view <n> --json statusCheckRollup` reports the latest build's result while a `-BuildId` you picked earlier describes an older one. Comparing "the failure scan" against "the check status" then compares two different builds and produces a contradiction that looks like a mystery — a leg reported SUCCESS while a failed task sat inside it. Extract the id from `detailsUrl` (`…/_build/results?buildId=<n>`) and confirm both halves of any comparison refer to the same build.
