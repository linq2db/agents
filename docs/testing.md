...
**Two gate attributes exist while [#5882](https://github.com/linq2db/linq2db/pull/5882) is in flight, and they behave differently.** `[ActiveIssue]` is the one described above: `AllowMultiple=false`, hides the test from discovery. `[ActiveIssueNew]` **runs** the test and rewrites the outcome — Inconclusive when it fails as declared, Failure when it passes or fails differently — and is `AllowMultiple=true`, so one method can carry a separate instance per provider, each with its own issue and expected failure. All of `Tests/EntityFrameworkCore` and a growing part of `Tests/Linq` are already on it; the rest migrate on that PR, after which the new one takes the old one's name. Practical consequences while both exist: a new gate on an already-migrated test is a **second attribute**, not an extended `Configurations`; a migrated gate should declare `ErrorType`/`ErrorTypeName`/`ErrorMessage`, because one with none matches *any* failure and will mask an unrelated regression; and a migrated gate whose issue is fixed announces itself as a failure saying "test passed but is marked", which is the point — `/enable-disabled-test` exists for the old attribute's inability to do that.
