...
**The same asymmetry run the other way is how you harvest what a gate is hiding.** To find out what a gated test *actually does* — which is the whole job when triaging whether a gate is still needed — select it **by name** (`--filter "<TestName>"`), which forces the `Explicit` case to run, and read the failure from a `--report-trx` file. No temporary edit to `ActiveIssueAttribute` is needed for a batch of a few sites — but it **is** needed for a whole provider's worth, because no filter selects them wholesale: `--filter "TestCategory=ActiveIssue" --provider YDB` discovered 158 gated cases, executed **4**, reported `skipped: 154` and exited **0**. Only a bare substring matching the *test name* runs them, so a cohort sweep means either one run per name group or temporarily dropping the `RunState.Explicit` write in `ActiveIssueAttribute.ApplyToTest` (never committed) and then filtering by category. An `--filter "Name~A|Name~B"` expression is the worst of the three: it selects no `Explicit` test at all, and `Name~` matches the **class** name too, so it returns a plausible count of entirely the wrong tests. Two traps measured on #5882 (2026-09-08): a **fixture** filter is not a substitute — `--filter "Issue1307Tests"` ran 252 cases of that fixture's one ungated test and *none* of its four gated ones, reporting a green run that says nothing about them; and `--list-tests` **does** list gated tests under a fixture filter, so a listing is not evidence that a run will execute them. **A third trap, measured 2026-09-09, bounds the name-filter route itself: it only reaches the gated cases when *every* case of that method is gated in the run you are making.** `--filter "MappingTypingByConstant_FromEnumerable_Int64"` — the exact method name — ran the four `.LinqService` cases and *none* of the four direct ones, because that gate carries `SkipForLinqService = true`, so the method had runnable and `Explicit` cases side by side and only the runnable ones executed. Every earlier harvest that worked by name had the whole method gated for the provider being run. So check the gate for `SkipForLinqService` / `SkipForNonLinqService` — or for a provider set narrower than the run — before trusting a by-name harvest, and reach for the `RunState.Explicit` edit when it is mixed. The tell is a run whose count is exactly the *ungated* half. Derive any `ErrorMessage` you then write from the trx's raw `ErrorInfo/Message`, never from a whitespace-normalised view of it: Shouldly wraps `should be` / `0` / `but was` / `1` across four lines, so a fragment copied from a collapsed rendering matches nothing (use the `{0}`-placeholder form, which `ThrowsWhenAttribute.MessageMatches` expands to `.*?` under `RegexOptions.Singleline`).
