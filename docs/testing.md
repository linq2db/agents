...
- **Declare `ErrorType` / `ErrorTypeName` / `ErrorMessage`.** A gate with none matches *any* failure and will mask an unrelated regression. It is also self-verifying in the other direction: a wrong `ErrorTypeName` turns the gate into a failure, so a green run after adding one is evidence the type is right.
