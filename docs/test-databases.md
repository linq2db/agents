...
**In-memory PDisks — the flag needs the literal `true`, and today's value is inert.** Both setup scripts (`Build/Azure/scripts/ydb.sh`, `Data/Setup Scripts/ydb.cmd`) pass `-e YDB_USE_IN_MEMORY_PDISKS=1`, which does **nothing**. The `local-ydb` image parses the variable with a strict `== "true"` compare in two places — `use_in_memory_pdisks_flag()` (`ydb/public/tools/lib/cmds/__init__.py`) and `_use_in_memory_pdisks_var()` (`ydb/tests/library/harness/kikimr_config.py`) — and its entrypoint always passes `--ydb-working-dir /ydb_data`, so any value other than `true` (**including unset**) leaves storage file-backed under `/ydb_data/pdisks/`. YDB's own reference doc documents the variable as `0` or `1`, which is wrong; the quickstart page uses `=true`. The comments the two scripts carry assert RAM-backed, fsync-free storage — that is not what runs (#5756). If it's fixed forward to `=true`, confirm it engaged (`/ydb_data/pdisks/` stays empty; the generated config shows `SectorMap:1:64`) before trusting any timing claim. RAM is not a concern at `=true`: `TSectorMap` holds sectors in a sparse `THashMap`, so usage tracks bytes actually written, not the declared 64 GB.
