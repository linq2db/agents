...
| A **`Write` / `ExitPlanMode` payload** — not a command — containing a `//./` or `\\` literal, e.g. quoting docker's `open //./pipe/docker_engine` error verbatim into a plan or doc | Elide or describe the path (*"the `npipe:` docker_engine endpoint"*) instead of quoting it; say in the text that it was elided so the next reader doesn't re-add it | `block-non-c-drive.js` scans **file content and tool arguments**, not just Bash command strings, so the rows above read as command-only and don't fire as a warning here. A plan file quoting a docker daemon error was rejected at `ExitPlanMode` with *"network share `\\.`"*, pointing at the plan's `plan` field — which looks like the guard has mis-identified the *file* rather than its text. Hit on #5864. |
| A `<letter>:` bigram inside a `--jq` expression or any quoted argument — `--jq 'map({n: length, f: .[0]})'` | Drop the offending key name (`file` not `f`), or do the grouping in the PowerShell tool over `--jq '.[].path'` output | Same whole-string scan: `f: .[0]` is read as drive spec `F:` and the call is blocked with *"drive spec F:"*. Any single letter followed by a colon does it, so short jq key names are the hazard — `a:`, `d:`, `f:`, `z:`. Hit on #5864 grouping a PR's changed files. |
