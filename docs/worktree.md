...
**The gate is auto-memory, not the session: when a project has no recorded worktrees root *and* naming convention, ask once — then apply it silently from then on.** So a project whose conventions are already recorded gets no question at all, and one whose aren't gets exactly one, before the first `git worktree add` ever made against it. Ask for **both** halves in that single question, because the root alone still leaves every directory name to the agent's discretion — and a slug is always *derivable* (from the branch, the issue, the PR number), so nothing ever forces a stop and the names get invented one defensible directory at a time until the root is full of them nobody recognises. Write both to auto-memory in the same turn; later sessions read the convention, derive the name from it and create the worktree without asking. Applies to a **throwaway probe** worktree exactly as much as to a branch worktree — the diagnostic directory is the one nobody will recognise later. (Surfaced 2026-08-26, after two worktrees — one branch, one ad-hoc probe — were created under a remembered root with agent-invented names.)

One property every recipe may rely on, whatever the root is: the worktree is **not** an ancestor of the primary clone, so nothing in the primary clone's tree is picked up implicitly.

**Whether the worktree sits *inside* the primary clone is a property of the chosen root, not an invariant — and it changes behaviour.** A nested root (`<primary-clone>/worktrees/<slug>`) makes the primary clone an ancestor, so ancestor-walking lookups reach it; a root outside the clone doesn't. `UserDataProviders.json` discovery is the case this decides (see the section below), so establish which layout you're in before applying a recipe that depends on it rather than assuming either shape. The check is a path comparison: `git rev-parse --path-format=absolute --git-common-dir` minus the trailing `/.git` is the primary clone root — the worktree is nested iff its path is under that.
...
**A worktree you did not create this session may be *ahead* of its PR — check before treating the PR head as the work.** The user keeps one worktree per open PR across sessions, so a worktree already in `git worktree list` can hold commits a previous session made and never pushed — typically on a **detached HEAD**, where there is no branch and so no upstream to compare against by name. `git status` says only *"HEAD (no branch)"*, and nothing in `gh pr view`, the PR's commit list or its CI state reveals them. Run `git -C <worktree> log --oneline --decorate origin/<branch>..HEAD` before syncing, reviewing, or describing "what the PR does", and create the local branch at the **worktree's** HEAD (`git -C <worktree> switch -c <branch>`) rather than at `origin/<branch>`, which silently drops them. (Surfaced on #5762: two unpushed test commits sat there, one of which moved the PR's new test off the process-wide `RemoteDataContextBase._configurations` cache key — so the pushed head still polluted the key every configuration-less remote context shares, and a review of the PR as published would have reported a defect whose fix already existed on disk.)
...
`TestConfiguration` finds this file by walking **up** the directory tree from the test assembly (`GetFilePath` in `Tests/Base/TestConfiguration.cs`), not from a fixed path. The walk traverses **ancestors only** and stops at the **first** ancestor that has the file — so the worktree's placement (see *`<worktrees-root>`* above) decides whether anything needs doing at all:

- **Nested worktree** (any path under the primary clone, e.g. `<primary-clone>/worktrees/<slug>`): the walk from `<worktree>/.build/bin/…` passes the worktree root, finds nothing there — only `UserDataProviders.json.template` is tracked — and keeps going up to the **primary clone**, where the real file is. Nothing needs copying: connection strings, the default `Providers` set and `BaselinesPath` all resolve exactly as they do for a main-repo run, and neither consequence below applies. The one case that still wants a local copy here is a worktree on a checkout **predating `--provider`** (pre-#5621 / `v6.3.0` and earlier), where trimming a worktree-root copy's `Providers` array is the only way to scope the run — see [`testing.md`](testing.md) → *Scoping a run to specific providers*.
- **Non-nested worktree** (a dedicated worktrees directory outside the clone, or a sibling of it): the primary clone is not an ancestor, so the walk runs to the filesystem root and finds nothing. Both consequences below apply.

A worktree-local copy always wins when one exists — it is the nearest ancestor — so seeding one is also how you deliberately override the primary clone's settings for a worktree-scoped run, in either layout.

Two consequences, **for the non-nested layout**:
...
2. **Run** — `/test run <filter> worktree <abs-worktree-path>`: `/test` passes `repoRoot=<worktree>` to `test-runner`, which runs `dotnet test --project <worktree>/<project> --provider <names> --test-progress` so the worktree's code is built and exactly those providers run. Prepend `CreateData.CreateDatabase` to the filter only when the test needs the full schema — a self-contained `CreateLocalTable` test doesn't, which avoids rebuilding the target DB.
...
`git worktree add` internally runs `reset --hard --no-recurse-submodules`, so **git itself** never populates the `.claude/` submodule: the new worktree would get an *empty* `.claude/` directory — and since an unpopulated submodule is not a deletion, `git status` there reads **clean**, so an agent would load no `AGENTS.md`, no `CLAUDE.md`, no skills, with nothing signalling the absence.

**Since [#5820](https://github.com/linq2db/linq2db/pull/5820) the `post-checkout` hook closes that hole, so no manual bootstrap step is needed** — `git worktree add` fires `post-checkout` with `$3 = 1` and cwd set to the new worktree, and `_refresh-corpus.sh` initialises the submodule there before falling through to its usual fast-forward. Verified end-to-end on git 2.55: `.claude/` comes up at the corpus tip, and the primary clone's copy (branch, `core.worktree`, status) is untouched. Three things still to know:

- **The hook that runs is the *primary clone's* copy, not the new worktree's.** `core.hooksPath = .githooks` is relative, and `git worktree add` resolves it against the invoking clone, so a primary clone sitting on a branch that predates #5820 runs the **old** no-op script and the new worktree comes up empty anyway — silently, since the old script's early return printed nothing. If a fresh worktree has no corpus, check `git -C <primary-clone> log -1 --oneline -- .githooks/_refresh-corpus.sh` before suspecting anything else.
- **The bootstrapped corpus is on a detached HEAD, so `git -C <worktree>/.claude push` fails** with *"You are not currently on a branch"* — `submodule update --init` checks out the recorded gitlink detached and the `--remote --merge` refresh merges into that detached HEAD rather than attaching. `git -C <worktree>/.claude switch master` fixes it in place (the clone already created the local `master` ref), but do it *before* committing: a commit made while detached is orphaned when you attach, and git reports it only as a passing *"you are leaving 1 commit behind"* warning.
- **`git worktree remove` needs `--force`** — unchanged, see the section above.

- **Manual fallback**, only when `core.hooksPath` isn't enabled in the primary clone. Clone the corpus into the worktree, borrowing the primary clone's objects so it costs almost no download:
...
- **`git submodule update --init` in a linked worktree is safe on modern git — the old warning here was wrong.** This doc used to say the submodule git dir is shared (`$GIT_COMMON_DIR/modules/.claude`) with a single `core.worktree`, so initialising from a second worktree repoints the primary clone's metadata at it. Measured on git 2.55.0.windows.4 against a purpose-built superproject: the linked worktree gets its **own** git dir at `.git/worktrees/<id>/modules/.claude`, the shared `.git/modules/.claude/config` `core.worktree` is byte-identical before and after, and the primary clone's `.claude` stays on its branch with a clean status. #5820's hook does this on every worktree, so the claim was also self-contradictory. Retired rather than reworded — but note `git help worktree` BUGS still discourages multiple checkouts of a superproject with submodules in general, and the version at which the per-worktree `modules/` layout arrived was not established, so an old git may still behave as the warning described.
- **Address the corpus directly from a worktree** — `git -C <worktree>/.claude <cmd>`, not `git submodule …` from the worktree root. Not a safety rule (see above), just the shorter path to the thing you actually want.
