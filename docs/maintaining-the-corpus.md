...
- **No machine-specific absolute paths.** Reference sibling clones relative (`../linq2db`, `../linq2db.baselines`, `../linq2db.docs`, `../linq2db.wiki`); the primary clone as "the primary clone" or the `<clone-dir>` placeholder (this clone's folder name); **worktree locations as `<worktrees-root>/<slug>`** (defined in [`worktree.md`](worktree.md) → *`<worktrees-root>` — where worktrees go*) — never a concrete worktrees directory and never a fixed relation to the primary clone, since that varies per setup; the user profile as `~/.claude/...`, never `C:\Users\...`. No `C:\GitHub\...` or other absolute local paths.
