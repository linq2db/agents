...
- **Update rule:** Two version sites with **different ceilings**:
  1. The plain `<PackageVersion Include="Oracle.ManagedDataAccess" />` entry in `Directory.Packages.props` is **capped at the latest 21.x** stable, and the cap is a **TFM incompatibility, not a test failure**: 23.x ships `lib/net472` where 21.x ships `lib/net462` (measured in the package cache, 21.23.0 vs 23.26.301), and both consumers of this entry are net462 — `Tests/linq2db.Providers.props` under `'$(TargetFramework)' == 'net462'`, and `NuGet/NuGet.csproj`. A flat bump is an `NU1202` restore failure, so there is **nothing to retest**; the cap lifts only when the netfx test TFM moves to net472, which is its own migration (`Tests/Directory.Build.props` + seven test projects, 13 `NuGet/*.csproj` tools folders, `$(LinqToDBT4SharedTools)`, ~25 hardcoded `net462` literals in `.github/workflows/tests.yml` / `build-job.yml` / `build-vars.yml` / `run-provider-tests.ps1`, and the TFM of shipped T4 tool assemblies).
     - The wording this replaced — *"versions 23.x produce test failures in linq2db (currently); do not bump past 21.x without an explicit retest pass"* — asks for a retest that cannot be run: no CI leg exercises netfx Oracle at all (`Build/Azure/netfx/` has no oracle config, and `j_Oracle1819` / `q_Oracle2123` / `t_Oracle1112` are all `enable_os_windows: false` + `enable_fw_netfx: false`), and the Oracle images are Linux-only so a Windows agent cannot host one. Corrected 2026-09-21 after the maintainer asked for the bump and the restore constraint turned out to be the real blocker.
  ...
  2. The `$(OracleManagedLinqPadVersion)` property (referenced by the LINQPad-side `VersionOverride` site) **may be bumped to the latest stable** of the 23.x line — the LINQPad driver's netfx TFM is `net472`, which is exactly why it can take the asset row 1 cannot. Single consumer: `Source/LinqToDB.LINQPad/LinqToDB.LINQPad.csproj:56`. The property collapses into row 1 the day that migration happens; until then it is the only thing keeping the two ceilings apart.
