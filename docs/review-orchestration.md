...
**Run the master control *before* offering an out-of-scope observation's disposition, not after.** The walk's OOS choices — promote-to-finding / track-issue / leave-as-FYI — turn almost entirely on whether the PR caused the thing. So presenting the options while that is still unprobed asks the user to pick between dispositions on evidence that does not yet exist, and the option text inevitably leans on whichever pass wrote the observation. Spend the control first and lead with its result; the disposition then usually picks itself. (Surfaced on #5833: a chained-set-operation observation was presented from an unprobed reading and promoted to in-scope, then had to be re-dispositioned to a tracking issue one turn later when the control showed `master` producing byte-identical SQL — 7 columns and the same slow-mapping fallback. The measurement was worth having either way, and doing it first would have saved the round trip. Filed as [#5881](https://github.com/linq2db/linq2db/issues/5881).)
...
- **measure** — run a probe whose outcome decides **whether there is a finding at all**, then re-present the item with the result and disposition it properly.
...
**Offer `measure` whenever a pass returned `could-not-verify` — and expect it to refute as often as it confirms.** `prove-with-test+fix` assumes the finding is real and writes a red test for it; `measure` is the option for an item whose *existence* is still open, and the two are not interchangeable. The trigger is explicit: a `leadStatus[]` entry of `could-not-verify`, a severity that turns on an unrun probe, or a finding derived from a baseline (per the *a baseline establishes what SQL was emitted, never what the query computes* rule). Present the probe, its cost, and what each outcome would mean, then re-present the item with the measurement in hand. Two things make this worth a disposition slot rather than ad-hoc judgement: the answer routinely **kills** the finding, and one probe often settles several items at once. (Surfaced on #5877, where the option was offered twice and taken twice: one Release census refuted a suspected wrong-diagnostic-id defect — the plan's recorded `20+2` split was a transcription error against the true `14+8` — *and* supplied the measurement a second finding turned on; a second probe refuted a suspected wrong-advice defect by showing the reported node was correct for a reason an earlier round had already adjudicated.)
