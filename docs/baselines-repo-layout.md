...
**A test-group set comparison often settles it before you run that diff.** Reduce both sides to test names — `git -C ../linq2db.baselines diff --name-only origin/master...origin/baselines/pr_<n> | sed -E 's|.*/([^/(]+)\(.*|\1|' | sort | uniq -c`, and the same over `git show --name-only --format="" <interim-sha>`. When every group in your delta also appears in the interim PR's, attribution is settled by set inclusion alone, with no second diff and no reasoning about drift. It is also the form that survives being wrong about *which* commit moved master: you are matching what changed, not asserting when. (On #5877 the delta's six groups — `CheckField6`, `LeftJoin5`, `EagerConcatWithSetsDifferentNullability`, `TestConditionalInProjection`, `FilteredLeftJoinFallbackChainSurvivesSetOperation`, `LengthFromNonTranslatable` — were all present in #5725's baselines commit, which had merged twelve hours earlier. A review subagent working the same delta by a different route concluded baselines master *had not moved*, and was wrong.)
