...
### Process-global query-cache assertions need `[QueryCacheTest]`

A test that asserts **process-global query-cache state** — an exact parameter count via `query.ToSqlQuery().Parameters`, a `Query<T>.CacheMissCount` / `GetCacheMissCount` delta, a compilation count, or any cost that only holds while a cache entry survives a loop — must carry `[Test, QueryCacheTest]`. Two independent pressures break such an assertion, and the attribute is the only thing covering both: it derives from `ParallelizableAttribute(ParallelScope.None)` so the dispatcher runs it on the exclusive write-lock lane, **and** it nulls `MaxEntriesOverride` for the test's duration so the 100-entry cap that netfx / 32-bit runs apply cannot trim the entry mid-test. `[NonParallelizable]` alone is **not** sufficient — it leaves the cap in force, and the resulting flake reproduces on netfx legs only (64-bit non-netfx legs keep the 10000 default and never trim). Mechanics: [`query-cache-mechanics.md`](query-cache-mechanics.md). Flag `MAJ` (it reddens CI non-deterministically). The 17 tests carrying `[Test, QueryCacheTest]` in `ParameterTests` are the pattern to match. For process-global state that is *not* the query cache — a static `Configuration.*` toggle, the `Common.Convert` registry, the `DefaultValue<T>` registry — plain `[NonParallelizable]` remains correct (`ConvertTests`, `DefaultValueTests`). (Surfaced twice. PR #5614 build 22305: `ParameterTests.Caching` asserted a cache-derived parameter count — `ToSqlQuery().Parameters.Count.ShouldBe(1)` — but was the one such test missing the marker, so a parallel lane promoted its cached plan to 2 parameters; fixed then with `[NonParallelizable]`, superseded by `[QueryCacheTest]` in #5785. Then build 23575: `ParameterReuse_ImpureExpression_CostDoesNotGrowPerBuild` shipped in #5733 with neither marker and flaked on the SQL Server 2005 + 2008 netfx legs — `5 should be 2`, a cache miss where a hit was expected. Fixed in #5912.)
