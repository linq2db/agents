...
**A `resolved-by probe` row must probe the step the decision actually depends on — an adjacent step's documentation is not evidence for it.** A mechanism usually spans two stages in sequence — register then resolve, parse then evaluate, write then read — and the stage that is *documented* is rarely the stage that can fail. Finding a doc comment for the second one reads as confirming the whole chain, so the row gets marked `resolved-by probe` while the load-bearing stage was never touched. That is worse than `OPEN`: an unverified assumption is now presented as evidenced, and every later reader — the critic, the reviewer, you in three days — takes it as settled. Give each stage in the chain its own row, and cite **behaviour you observed** (a run, a test, the code path itself) rather than a comment describing intent. (Surfaced on [#5640](https://github.com/linq2db/linq2db/pull/5640): `U-1` asserted that a configuration-scoped mapping attribute is resolved in preference to an unscoped one, `resolved-by probe`, citing `MappingAttribute.cs`'s own doc comment. That comment describes attribute **resolution** and was accurate — but the decision rested on attribute **registration**, where `FluentMappingBuilder` looks up by attribute type with no configuration filter and *mutates the unscoped attribute* instead of adding a scoped one, which is the exact opposite of what the plan assumed. `plan-critic` refuted the mechanism; unrefuted, it would have applied one value to every provider while reading green on most legs.)
...
**Write rows the way `-Action reconcile` reads them.** It takes the **first** backticked path of a row (split at `:`), matches by equality, suffix or — for a path ending in `*` — prefix, and does **not** expand globs: `Tests/**/*.cs` authorizes nothing, `Tests/*` authorizes the tree. So give each file its own row (`E-17`, `E-17b`, …) rather than listing three paths in one. And on a **stacked** branch pass `-Base <parent-branch>` — the `origin/master` default diffs in every commit the parents carry and reports them as unplanned (#5969: 493 considered against the real 123).
...
An obligation that needs the harness configured in a way no existing test needs — an option forced on for a whole run, a provider matrix, a TFM the suite does not normally exercise — **owes a `P4` unknown naming that capability**. `P8` is written in terms of what will be asserted, which hides whether the run can be set up at all; the gap then surfaces at execution time, when the cheap answer (write the obligation differently) is no longer available. Ask of each obligation: what must be true of the runner for this to be executable? (2026-09-14: a bulk gate specified as "the whole suite with `PreferClientCalculation` forced on" turned out to have no harness hook, and needed a temporary edit to `TestBase` that could never be committed.)
