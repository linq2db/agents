...
**A bot review can land *after* the context load — re-query before finalizing the audit.** Copilot reviews on a **push**, not on PR open, and it posts within a few minutes of one. So when the PR head was pushed shortly before the session started, `pr-context.ps1`'s `reviews[]` legitimately predates the bot's review and the audit silently covers only the human reviewers. Before assembling `## Prior-review audit`, re-run `gh api repos/<owner>/<repo>/pulls/<n>/reviews --jq '.[] | {id, login: .user.login, state, submitted_at}'` and compare against the loaded set; if the head commit's date is within ~10 minutes of the load, treat a re-query as mandatory. **Note the login mismatch when filtering:** REST reports `copilot-pull-request-reviewer[bot]` while the GraphQL/`gh pr view` shape reports `copilot-pull-request-reviewer` without the suffix, so a filter written for one silently matches nothing on the other. (Surfaced on #5643: the context load ran at 19:01:52Z, Copilot submitted at 19:05:14Z — 3m22s later, triggered by a master-merge push. The audit covered only the human reviewer, and Copilot's two findings, one of them a real defect that threw for constructor-mapped entities, surfaced only because the user asked for them by name. The first diagnosis blamed `pr-context.ps1`; the script is correct — the review simply did not exist yet.)
...
- **Source-file encoding / BOM claims.** When a bot claims the PR added or removed a file's UTF-8 BOM (or changed its `charset`), read the cached `_diff/<path>.diff` — the per-file diff preserves the BOM marker in its context lines (a removed BOM shows as `-﻿using …` → `+using …`), so it answers the claim directly. Do **not** `xxd` / `head -c` the cached HEAD body: the cache writer may normalize encoding (stripping the very BOM you're checking for), and the raw byte-pipe prompts on the allowlist. Surfaced 2026-05-31 on PR #5542 (a Copilot BOM-removal claim confirmed Still-actual from the `_diff`, after a redundant `xxd` pass). **Once the `_diff` confirms an add/remove, classify it against the repo convention rather than re-checking bytes:** `.cs` / `.csx` / `.vb` / `.vbx` sources in this repo carry a UTF-8 BOM by convention, so for those file types a BOM **addition** conforms (a bot flagging it as scope-creep is **Inaccurate**, not a finding) and a BOM **removal** deviates (a legit finding). Settle it from the convention as stated here, never an `od` / `head -c` / `git show … | od` pass over a sibling file — the byte-pipe prompts on the allowlist and re-derives a convention this doc already states. Surfaced 2026-05-31 on PR #5525, where the PR's BOM additions (`QueryFilterAttribute.cs` plus two new files) were correct, confirmed only after a redundant `git show … | od`.

> The convention is **not** enforced by `.editorconfig` — an unenforced `charset = utf-8-bom` rule was dropped from it, and `.editorconfig` sets no `charset` today. Don't cite `.editorconfig charset` as the anchor (it resolves to nothing), and don't read the drop as "the BOM convention was abandoned" — the sources still carry BOMs.
...
4. `dotnet build Tests/Tests.Playground/Tests.Playground.csproj -c Testing` then `dotnet test … --filter FullyQualifiedName~<TestName> --test-progress`.
...
When the audit classifies a thread as Still actual AND the corresponding concern is **not** present in `code-reviewer`'s `findings[]` / `out_of_scope_observations[]` for the same area, append one JSON-line entry to `.build/.agents/review-quality-signal.jsonl` (create the file if absent):
...
The file is gitignored (`.build/` is) and serves as the input corpus for periodic rubric tuning — the same shape as `.build/.agents/copilot-vs-review-gap-report.md` produced 2026-05-11. Do **not** surface these entries in the review body or to the user during this run; they are passive accumulation. The `chores` / `audit-agents` skills harvest the log on a separate schedule. (When neither Copilot nor Codex thread on a PR is Still-actual, no entry is written — the absence of the log line is the "review skill caught everything" signal.)

## Thread dispositions

Batched through `post-pr-thread-replies.ps1 -ManifestFile .build/.agents/pr<n>-thread-replies.json`, one allowlisted call:
