...
**Two sanctioned exceptions.** `/review-pr` step 4's second question asks the user whether to run the baselines review *at all*, and a sensible recommendation needs two facts **before** the subagent is spawned.

1. **Branch presence.** A single `git -C ../linq2db.baselines rev-parse --verify --quiet origin/baselines/pr_<n>` (run after the `fetch` above; a non-zero exit means the branch is absent).
2. **Branch age.** A single `git -C ../linq2db.baselines log -1 --format="%cI" origin/baselines/pr_<n>`, compared against the PR's HEAD commit date from the context load. `rev-parse` cannot answer this and the subagent's diff won't flag it: a branch written before the PR's latest commits describes SQL the current code no longer emits, so the pass can raise findings those very commits already fixed. See [`baselines-repo-layout.md`](baselines-repo-layout.md) → *Commit granularity* and `/review-pr` step 4 Q2.
