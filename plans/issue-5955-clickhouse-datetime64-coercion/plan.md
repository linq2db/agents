...
### D-1 — Coerce **unconditionally** with `toDateTime64(x, 7)` — **SUPERSEDED, see A-3**
...
- A-3 (2026-09-22) — **`D-1` superseded: the coercion is conditional, because the reason it could not be is fixed rather than worked around.** Raised by the user on review: *"AsDateTime64 helper - isn't SqlCast do same and also provides reduction when source and target types match?"*, then *"we have bad typing of casted expression? then isn't it a bug itself?"*. Both hold.

  **The cast renders identically.** `BasicSqlBuilder.BuildSqlCastExpression` → `ClickHouseSqlBuilder.BuildTypedExpression` looks the target `DataType` up in `ClickHouseConvertFunctions` and emits `toDateTime64(x, p)`; there is no `CAST(… AS …)` path for these types, and `ClickHouseSqlExpressionConvertVisitor.MakeConversion` already builds this exact node for a `DateTime64` target. `D-1`'s rejection reason ("the builder has to translate a CAST back into a function") was false.

  **The typing was the bug, and it is three bugs, not one.** Of the four operand kinds in `P1`, only two were mistyped in a way that matters: `now()` / `makeDateTime` (provider-local - typed from the mapping schema, which maps a CLR `DateTime` to `DateTime64(7)`, while the server answers `DateTime`; measured on 26.5.7.64 via `toTypeName`), and `MAX`/`MIN` (`U-1`, filed as #5960). The `MAX`/`MIN` half needed no new mechanism: `SqlArgumentDomains.ForAggregate` already marks them `SqlArgumentDomain.Element` - *"the result is one of the argument's own values … so the argument's column describes it completely, width included"* - and `QueryHelper.GetColumnDescriptor:460-466` already follows that arm for the value-converter path. Only `GetDbDataTypeImpl` lacked it.

  **The cast's own reduction is not the mechanism, and cannot be.** Measured, not reasoned: with a non-mandatory cast whose target is identical to its operand by construction, `toDateTime64(t.DateTimeValue, 3)` over a `[Column(Precision = 3, Configuration = ProviderName.ClickHouse)]` column still reached the SQL - same `DataType`, same precision, no `DbType`/length/scale on either side, and `factory.GetDbDataType` *is* `QueryHelper.GetDbDataType(expr, MappingSchema)`, the same call the strip makes. Both site families behave this way. For the convert-visitor sites there is a visible cause - `OptimizationContext.OptimizeAndConvertAll` runs the optimizer *before* the convert visitor, and `VisitSqlIntervalDifferenceExpression` / `VisitSqlTemporalArithmeticExpression` hand their result to `Visit(lowered)` rather than `Visit(Optimize(lowered))` - but the translator sites are built before optimization runs and are not stripped either, which that cause does not explain. Recorded as an open observation, not a claim, and filed as #5963 (6.6.0) with the evidence; not this branch's to settle.

  So the helper tests the operand itself: already `DataType.DateTime64` → returned unchanged, anything else → `factory.Cast(expression, DateTime64(7))`. `E-8` adds the `QueryHelper` arm, `E-9` the two ClickHouse types, `E-10` rewrites `E-5`'s body. `P3`'s "No change to `QueryHelper.cs`" anti-goal is void by explicit user decision, taken with the cross-cutting blast radius stated. `A-2`(ii)'s `max(operand precision, 7)` is void with it - an operand that is already `DateTime64` is no longer converted at all, so there is no precision to choose. New obligations: `TO-12` the coercion is emitted only where the operand needs it (both halves - absent on a default-mapped column, present on `now()`, `makeDateTime` and a declared `DataType.DateTime` column), and `TO-13` a core-layer pin that `MIN`/`MAX` report their argument's `DbDataType` while `SUM` keeps its own.
