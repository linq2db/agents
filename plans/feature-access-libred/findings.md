...
| R10 | ~~A parameterised stored SELECT cannot be a table source~~ — **not a defect** (re-measured 2026-09-24 against ACE) | `SELECT [FirstName] FROM [Person_SelectByKey]` with `@id` supplied works on both, aliased / filtered / joined too; without it both refuse (ACE: *No value given for one or more required parameters*). The alpha.3 error was the probe not supplying `@id` | — | fixed in alpha.3 |
| R11 | ~~`IS TRUE` / `IS [NOT] DISTINCT FROM`~~ — **not a defect**: ACE rejects both (*Invalid use of IS operator* / *Syntax error*), grammar has only `IS [NOT] NULL` | — | — | — |
| R12 | `WITH OWNERACCESS OPTION` rejected — the only item of the old list ACE accepts (measured 2026-09-24). ACE also rejects `CAST`, `LIKE … ESCAPE`, `TOP … WITH TIES`, `NZ()`, `{ts}` escapes and `VALUES` as a source; LibRed *accepts* `FROM (VALUES …) AS v` and `UNION ALL VALUES` (grammar `ValuesTerm`), just not a column-alias list | `SELECT … WITH OWNERACCESS OPTION` → `mismatched input 'WITH'` | `AccessHints.WithOwnerAccessOption` unusable | narrowed |
