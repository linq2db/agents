...
- G-05: **pass** — `dotnet build Source/LinqToDB/LinqToDB.csproj -c Release -f netstandard2.0` → exit 0, and `-f net462` → exit 0. Release, so analyzers and banned-API checks ran. Load-bearing here because E-1 exists precisely for the TFMs where the BCL enum lacks member 35.
