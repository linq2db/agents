...
- A-25 **DB2 cohort: `A-15`'s quarantine rule fires a second time, and one gate's prose finally survives (2026-09-08).** Of eight sites, four fail with `IBM.Data.Db2.DB2Exception : ERROR [4xxxx] A codepage conversion problem occurred creating this message` — the identical UTF-8-ACP defect `A-13` root-caused on Informix, masking the server's real error, so `D-16`'s check cannot even be attempted and the local result is inadmissible. Two more sit inside `#if AZURE` and are not compiled in `Testing`; their prose is "Fails on CI", so local evidence would be inadmissible even if they were. That takes the CI-sweep bucket to **18** (9 Oracle-Devart, 3 Informix, 6 DB2), which is now large enough that batch C's CI run is the only thing that can close it. **Method note that generalises past DB2:** the quarantine trigger is not "the provider is awkward" but "the failure text names a *client-side* conversion, locale or codepage error", and it is decidable from the first line of the message — worth checking before any analysis, because everything downstream of it is wasted.
  The two admissible sites split the way this whole phase keeps splitting. `RatioToReportZeroPartitionSum` cited **#5663** (closed, and about mapping a DECFLOAT special value to *decimal*); measured, the reader maps it perfectly — the column is a `double`, materialising as `∞` / `-∞` — and the assertion fails because the test wants `null`. DB2 has a *native* `RATIO_TO_REPORT`, so linq2db emits it directly and it never reaches the `NULLIF` emulation that gives every other provider a NULL on a zero partition total. Filed as **#5897**. `Round4Sql`, by contrast, is the **first gate this session whose stated cause survived measurement** — and it survived only because the probe was run: linq2db emits a banker's-rounding `CASE` rather than a plain `ROUND`, so the emulation was the obvious suspect, and it is exonerated (its midpoint branch is never taken for these values, and DB2's own `ROUND(CAST(6.6 AS Float), 5)` returns `6.6000000000000005` unaided). Running one to five refuted gates against one upheld one is the honest tally for `D-16` V1, and the upheld one is the case that would have been cheapest to wave through.
- A-26 **The narrow cohorts are exhausted: four last sites, and two of them split (2026-09-09).** Both `MappingTypingByConstant_*_Int64` gates now cite **#4955** and declare the measured `OverflowException` — the test feeds `2147483648L`, a missing `CAST … BIGINT` returns it as an `int`, and it overflows on materialisation. `TestMergeTypes` and `UpdateIssue321Regression` were each *one* gate covering two providers for **unrelated** reasons, which `AllowMultiple = true` exists for: the YDB halves are measured against #5591, and the other halves say plainly that they cannot be reached (`Oracle.21.Devart.Direct` needs a licence key this box lacks — and the gate's prose never covered that provider at all, so it carries `no-declaration: unvalidated:`; `ProviderName.Sybase` is netfx-only, so its declaration is carried from prose under `unvalidated:`). **A two-provider gate whose `Details` names a different cause per provider is a split waiting to happen, and the old attribute could not express it** — worth scanning for in the remaining population rather than discovering per site.
- A-27 **Three measurement traps hit in one cohort, all of which produce a confident wrong answer rather than an error.** (1) **The by-name harvest has a boundary `E-29` did not state.** It reaches a gated case only when *every* case of that method is gated in the run being made: `--filter "MappingTypingByConstant_FromEnumerable_Int64"`, the exact method name, ran the four `.LinqService` cases and none of the four direct ones, because the gate carries `SkipForLinqService = true`. Every earlier successful harvest had the whole method gated for the provider under test, so the rule had looked unconditional. The tell is a run whose count is exactly the *ungated* half; `testing.md` now carries it. (2) **A failed build was read as a test result.** `MSBUILD : error MSB4223: A node of the required type OutOfProc could not be created` killed the build, the run proceeded against a **twelve-minute-old binary still carrying the sweep instrument**, and the four direct cases came back `Failed` with the raw exception — which reads exactly like "the new attribute is not governing them". `-m:2` fixed the build and the same run returned Inconclusive. The binary timestamp against the source timestamp is what settles it, and it is worth checking whenever a verification's *shape* is wrong rather than its content. (3) **The census counts an unresolved token as one provider**, so `AggregatesOverADifference` presented as a 2-provider gate when its two fixture-local `const string` composites expand to Access + Informix + SqlServer2014Minus — **fourteen**. Anything still showing `UNRESOLVED:` must be read before being called narrow.
- A-28 **A gate can be stale for part of its `[Values]` matrix, and neither the old attribute nor the CI sweep can see it (2026-09-09).** `JoinTests.cs::Issue3560Test4` was gated wholesale, but only the `LikeClr` mode fails — `LikeSql` and `LikeSqlExceptParameters` pass, so the new attribute turned two working cases into `Test passed but is marked`. Two reasons this class is invisible until a migrated site is actually run: `ActiveIssueNew` targets a **provider** or a **transport**, never a test-case argument, so `[Values]` staleness is inexpressible; and the CI sweep emitted **no `##L2DB-AI` sentinel lines at all** (it ran the un-hide instrument on the old attribute, not the new one's sweep mode), so it recorded failures only — a partially-stale gate is byte-identical to a fully-failing one in that harvest. Only a *wholly* stale gate is caught by the harvest, because it yields no evidence row. **Disposition (user, option `b`): restructure the test, don't extend the attribute** — the ungated method keeps the passing `[Values]` subset, a sibling method carries the gate for the failing value, and both call a shared core so the body is not duplicated. **21 of the remaining old-attribute sites also carry a `[Values]`/`[TestCase]` parameter** (`values-sites.json`), so every one of them needs a post-migration run before its batch is committed; assume nothing from the CI evidence alone.
- A-29 **Shouldly renders the same assertion differently on CI and locally, so a Shouldly fragment harvested from CI cannot be declared verbatim (2026-09-09).** CI loses the caller expression — `Shouldly.ShouldAssertException : 0d / should be / 2d / but was not` — where a local run has it: `rows[1] / should be / 2d / but was / 0d`. A declaration taken from either one fails against the other. The environment-independent shape is the expected value with its two neighbours, `should be{0}<expected>{1}but was`, which the branch's earlier Shouldly gates already use; the two `ZonedDifference*` gates were authored in the CI-only shape and corrected. Verified against `Oracle.11.Managed`, `Oracle.12.Managed` and `SQLite.Classic`. Related and separately load-bearing: the same measurement showed the **two providers fail on different rows and different members** (SQLite `rows[1]`/`row.Days`, Oracle `rows[0]`/`row.Hours`), so the split into per-provider attributes was necessary rather than cosmetic — a single shared declaration would have been wrong for one of them.
- A-30 **`MappingTests.cs::StorageFieldTest`'s premise is wrong, and the obvious repair is worse (2026-09-09).** The mapping sets no `Storage` at all — `[Column] public int Field => _field;` — so the test asks linq2db to discover a get-only property's backing field, which is not what its cited #279 reports (that issue is closed as delivered by value converters). Probed per the user's call: adding `Storage = "_field"` makes the generated DDL grow a **third** column, `[_field]` beside `[Field]` and `[Id]` (measured against the CI DDL, which has two), so the private field is mapped as a column of its own as well as being the property's storage. Direct SQLite then passes and the `LinqService` case fails one assertion later on the update. Probe reverted; the gate stays as authored, declaring the wrong-value failure it actually produces. The phantom-column behaviour is a product finding with no tracker and was **not** chased to its code path — that would be `Source/` work, which `P3` excludes from this branch.
- A-31 **`ActiveIssueNew` gains a `Platforms` dimension: staleness has a second axis, and it is the OS (2026-09-10).** `SQLiteTests.cs::TestDoubleRoundTrip` fails on CI across `SQLite.Classic`, `.MPM`, `.MPU` and `SQLite.MS` and **passes locally on the same four**. Identical provider list, opposite outcome, so it is platform- not provider-dependent: the bundled SQLite native binary differs per RID and only the Linux one loses the last two digits of `CAST(… as real)`. `A-28`'s harvest blindness applies here for a different reason — each CI leg runs exactly one OS, so the sweep can never see the disagreement. Where `A-28` was resolved by restructuring the test, the user chose option `b` here: a `TestPlatform` flags enum plus a `Platforms` property, checked in `AppliesTo` **before** the `provider == null` shortcut (otherwise a platform bound is silently inert on exactly the tests that have no other targeting) and folded into `HasExplicitTargeting` so a platform-scoped attribute outranks a blanket one. Eight self-tests added, including the case that matters — when the *only* attribute is for another platform, `SelectGoverning` returns null and the passing result is left alone rather than rewritten to "passed but is marked". Verified: 43/43 green on Windows with the gate carrying `Platforms = TestPlatform.Linux`. `MacOS` is deliberately not declared — unmeasured, and no leg runs it.
- A-32 **`OracleTests.cs::Issue399Test`: the gate is obsolete and the test's expectations are stale environment data (2026-09-10).** No CI evidence exists (Oracle has no GitHub-CI leg), so it was measured against `oracle11`: `Assert.That(schema.Procedures, Has.Count.EqualTo(11))` gets **31**. #399 is closed and reads *"Oracle T4 templates missing packages and certain stored procedures"* — the opposite complaint. Probed by dumping the rows: seven are package members (`ISSUE2132`, `TEST_PACKAGE1`, `TEST_PACKAGE2`), so the packages #399 asked for **are** loaded, and the six rows with no schema are *present* carrying a `ResultException` rather than missing — the T4 filter in the second assertion excludes them by design, and now yields 25. Both hard-coded `11`s are counts of whatever the Oracle test schema held when the test was written. **Disposition (user, option `b`): `remove`.** The gate is gone and the counts are replaced by assertions on what #399 asked for — `TEST_PACKAGE1` and `TEST_PACKAGE2` each describing their three members, and the T4 filter keeping exactly the procedures whose result schema could be read, which the dump proves equivalent to `ResultException == null` across all 31 rows. Green ungated on `Oracle.11.Managed`; `Data/Create Scripts/Oracle.sql` is shared by every Oracle version, so the package assertions do not depend on which one runs. **Generalises:** an assertion on a *count of schema objects* is environment data wearing a test's clothes, and a gate can outlive the defect while the count keeps it red.
- A-33 **Two more silent-miss shapes, both found by running what the harvest had already "answered" (2026-09-10).** (1) **An `UNRESOLVED:` provider list joins to nothing, and the join reports that as "no CI evidence".** `AggregatesOverADifference` presented as evidence-free; its `Configurations` are two fixture-local `const` composites, the join is keyed on provider, and going to the raw failure index directly found nine failures saying exactly what the gate's prose claimed. Read `UNRESOLVED:` as *not joined*, never as *not failing*. (2) **A provider named in both a gate and its test's own exclusion list is dead weight.** `CteTests.cs::TestUpdate` gated `TestProvName.AllOracle` with `Details` explaining Oracle's CTE+UPDATE syntax, while its data source is `[CteContextSource(…, TestProvName.AllOracle, …)]` — an exclusion list — so no Oracle case was ever built. Both are worth a sweep rather than per-site discovery.
