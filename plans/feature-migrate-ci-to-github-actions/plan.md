...
- U-10 Whether an x86 apphost on a GitHub Windows runner resolves a runtime installed by `setup-dotnet` with `architecture: x86` — the action's input exists on the pinned v6.0.0 (`a98b568…`, checked against its `action.yml` at that SHA), but it sets `DOTNET_ROOT` while a 32-bit apphost on 64-bit Windows reads `DOTNET_ROOT(x86)` first — resolved-by probe, TO-11, both arms
- U-11 Whether a `test-all` fits GitHub Free's 20-concurrent-job cap once the Windows legs land: 12 Linux + 5 Windows + `prepare` + `build` + `build_x86` = 19, before any concurrent `build.yml` — resolved-by measurement on the first full run, TO-15
