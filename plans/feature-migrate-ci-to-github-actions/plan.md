...
- E-38 (phase 3 follow-up, added by A-4) `Build/Azure/pipelines/templates/test-jobs.yml:test_windows_job,test_ubuntu_job` — the job `condition` additionally requires `ne(variables['title'], '')`, so a job whose matrix selected nothing is skipped rather than reporting a green run of no tests
