...
- U-2 Whether the ~4 min per-leg win-mssql image pull makes SQL Server the wrong leg set to park on the slower agents — **resolved-by measurement, A-6**: a full run exists, and the windows legs are now Azure's entire critical path
- U-3 Whether artifacts survive a partial re-run so a re-run leg can download `build`'s output from attempt 1 — **resolved yes**, A-7: `gh run rerun --failed` on run 34295953589 re-ran `Win r_Access_MDB` alone against attempt 1's artifacts and went green
...
- U-10 Whether an x86 apphost on a GitHub Windows runner resolves a runtime installed by `setup-dotnet` with `architecture: x86` — **resolved yes, both arms**, A-7: with the step `DOTNET_ROOT` is `C:\Program Files\dotnet\x86` and 14484 net10 x86 tests pass; without it (control branch, run 34295367991) `DOTNET_ROOT` stays at the x64 root and every x86 leg dies loading the x64 `hostfxr.dll`, `HRESULT: 0x800700C1`
- U-11 Whether a `test-all` fits GitHub Free's 20-concurrent-job cap once the Windows legs land: 12 Linux + 5 Windows + `prepare` + `build` + `build_x86` = 19, before any concurrent `build.yml` — **still open**: no phase-4a probe exceeded 6 concurrent jobs, so TO-15 waits for the first `test-all` after E-44
...
- E-28 (phase 4, **done for the docker-free half**, A-7) `.github/workflows/tests.yml` — add the `windows-tests` matrix job; the eight win-mssql legs stay on Azure per D-2 and keep the job alive there
- E-40 (phase 4a, added by A-7) `Build/Azure/pipelines/templates/test-matrix.yml` — `gh_windows: 'true'` on the five docker-free entries (`g_SqlCE`, `r_Access_MDB`, `s_SQLite`, `v_Access_ACE_Odbc`, `x_Access_ACE_OleDb`), an `offload_windows_to_github` parameter declared and forwarded, and both `gh_linux` and `gh_windows` documented in the MISC block — `gh_linux` had only an inline note in `test-jobs.yml`, which is not where a reader looks up matrix schema
- E-41 (phase 4a, added by A-7) `Build/Azure/pipelines/templates/test-jobs.yml` — `offload_windows_to_github` in the typed-list parameters, and the same `not(and(…))` skip on `test_windows_job`'s matrix the linux job already carries. **No caller passes it in this PR** — see the sequencing note in A-7; `default.yml` never will (D-6)
- E-44 (phase 4b, **done**, A-7) `Build/Azure/pipelines/testing.yml` — `offload_windows_to_github: true`, [#5899](https://github.com/linq2db/linq2db/pull/5899) / `cf9fa573c`, landed after E-43 was on master
- E-42 (phase 4a, added by A-7) `Build/CI/run-provider-tests.ps1` — new, the Windows counterpart of `run-provider-tests.sh`; config stage, local cmd/ps1 setup, pre-test, main (optional) and EF.Core suites as apphost `.exe`, post-test, `finally` cleanup
- E-43 (phase 4a, added by A-7) `.github/workflows/tests.yml` — `prepare` also emits a `windows` matrix plus `windows_any` / `windows_x86`; `build` stages netfx x64 when a windows leg was selected; new `build_x86` job, gated on `windows_x86`, publishing **one artifact per TFM** rather than Azure's single win-x86 artifact
...
- E-38 (phase 3 follow-up, added by A-4) `Build/Azure/pipelines/templates/test-jobs.yml:test_windows_job,test_ubuntu_job` — **withdrawn by A-5**, refuted empirically by build 23453
- E-39 (phase 3 follow-up, added by A-5) `Build/Azure/pipelines/templates/test-workflow-linux.yml` and `test-workflow-windows.yml` — one step per template, the only one not guarded on `title`, naming the empty-matrix state in the timeline instead of leaving a bare green job
...
- TO-10 (phase 4a) A hand-dispatched `[access.all]` selects exactly `r_Access_MDB`, `v_Access_ACE_Odbc`, `x_Access_ACE_OleDb` and matches the last Azure `test-access` run's per-leg test counts — proof: characterization
- TO-11 (phase 4a) The x86 apphost resolves a runtime (U-10) — proof: control, the same leg with and without the `architecture: x86` step; without it the net8/9/10 x86 suites must fail to start
- TO-12 (phase 4a) `[sqlce.all]` produces `.trx` for netfx + net8/9/10, main and EF.Core, and a `[Windows / SQL CE]` baselines commit — proof: characterization
- TO-13 (phase 4a) `[sqlite.all]`'s windows leg runs **netfx EF.Core only** — proof: control against the Azure leg's step list. This is where `win_efcore_only` and the ubuntu-derived TFM rule are wrong or right and nowhere else
- TO-14 (phase 4b, gates E-44) With `offload_windows_to_github: true` an Azure `test-all` lists exactly the eight win-mssql windows legs; with it off (the `default.yml` path, and phase 4a's state) all thirteen — proof: control. The D-6 guard: a release run must not lose the five legs
- TO-15 (phase 4a) A GitHub `test-all` peaks at ≤ 20 concurrent jobs (U-11) — proof: measurement, job start times off the run
...
- **SQL Server on the slower agents is unmeasured** (U-2). D-2's split is by capability, not duration. Do not flag it as a load-balancing defect before a full run exists; do flag it after. — **now flagged, A-6**: the windows legs run 1h2m–1h12m each on Azure against a 67-min pole for GitHub's entire linux set, so they are the whole critical path.
...
- A-7 (2026-09-09) **Phase 4a: the five docker-free windows legs move to GitHub** — `g_SqlCE`, `s_SQLite`, `r_Access_MDB`, `v_Access_ACE_Odbc`, `x_Access_ACE_OleDb`. Adds **E-40**..**E-43**, **U-10**, **U-11** and **TO-10**..**TO-15**; branch `feature/gh-windows-nondocker-legs`. Azure's windows job drops from 13 legs to 8, which A-6 identified as where the remaining wall-clock win lives: 13 legs over 10 slots is two waves, 8 is one. `w_DB2InformixDuckDB`'s windows leg is left alone — `enable_os_windows` is emitted only under `full_run`, full runs are `default.yml`, and offload is off there, so marking it would be inert. **E-29/E-30 stay out of scope**: Azure must retain the whole windows capability for the release run, so `build_x86_job` and its `dependsOn` remain. It is now unused on PR runs, costing a slot for ~7 min in a window `build-job.yml`'s own comment measures as *not* the binding constraint (it finished 0.6–1.9 min ahead of `build_job`, which `create_baselines_branch` then follows by 2–3 min).

  Three things the port could not copy verbatim, each a decision rather than a translation. **Artifacts are per TFM, not Azure's single win-x86 one**: `download-artifact` has no `itemPattern`, so one artifact would force every leg to take all four TFMs at once and lose the download-run-delete disk discipline. **The suites are apphost `.exe`**, so unlike the linux script there is no `dotnet <dll>` hop — which is what makes U-10 (the x86 runtime) a question at all. **The TFM derivation is restated in Python**: `test-jobs.yml`'s "windows runs net8/9/10 only for a leg linux cannot cover, unless `win_tfms_always`" rule now exists in two languages, and the only thing keeping them in step is TO-13, which is the single leg (`s_SQLite`) where the two arms disagree.

  Both new guards in `prepare` were shown to **fire**, against mutated copies of the matrix rather than by reading: marking `w_DB2InformixDuckDB` `gh_windows` errors on its `${{ if … }}` key (a stock parser sees a key literally named `${{ if eq(parameters.full_run, true) }}`, so `enable_os_windows` reads as `None` and the leg would vanish silently — D-3's failure mode by a fourth route), and clearing `s_SQLite`'s `enable_fw_netfx` errors on a windows leg with no TFM enabled. The selector itself was run — the shipped code, extracted from the workflow rather than a copy — over eight surfaces, and `run-provider-tests.ps1` over a fake leg tree with stub executables (retry loop 3/3, efcore-only path, and all three guards). Everything else is CI-only and gated on the probes: `[sqlce.all]`, `[sqlite.all]` and `[access.all]` dispatched against the branch **before** `testing.yml`'s flip is trusted.

  **Both halves merged: [#5898](https://github.com/linq2db/linq2db/pull/5898) (`ebba22518`, the workflow) and [#5899](https://github.com/linq2db/linq2db/pull/5899) (`cf9fa573c`, the flip). The split is live for PR runs.** One obligation is still open and it is the Azure half: **TO-14 has not been run.** Every leg was proven on the GitHub side before merge, and the shipped selector re-checked against merged `master` still returns the five, but nothing has yet confirmed that Azure *stops* running them — nor that the eight win-mssql legs are unaffected. The matrix condition is the same construct `test_ubuntu_job` already uses, which is exactly the reassurance A-5 warns against: an Azure expression is proven only in the position it already occupies, and this is a new position. Over-skipping would silently drop the win-mssql legs; under-skipping merely duplicates work and is visible. Settle it with two cheap `/azp run` comments on any open PR — a moved surface (`test-access`) must show no windows leg on Azure while GitHub runs three, and a retained one (`test-sqlserver-2022`) must still show its windows leg on Azure. Read the leg list from the timeline, never the build verdict.

  Shipped as draft [#5898](https://github.com/linq2db/linq2db/pull/5898), milestone 6.5.0. TO-10, TO-12 and TO-13 pass; TO-11 passes with both arms; U-3 and U-10 close.** Hand-dispatched: `[access.all]` 3 legs green with 28748 tests / 0 failures over six `.trx` on `r_Access_MDB`; `[sqlce.all]` 16115 / 0 over six; `[sqlite.all]` green with the windows leg running **netfx EF.Core only** (147 tests), which is TO-13 exactly; `[oracle.all]` green with every job skipped. One flake, not reproduced: `DataConnectionCommitTransactionAsync("Access.Jet.OleDb")` failed once with `Query must have at least one destination field` and passed on re-run at the same commit, alternating with Inconclusive-and-skipped across runs.

  **Three defects surfaced during bring-up, none of which reading the diff would have caught, and each is a reusable lesson for the docs after merge.** (1) **GitHub evaluates `${{ … }}` anywhere in a workflow file — comments inside a `run:` block included.** A Python comment quoting Azure's own `${{ if eq(parameters.full_run, true) }}` failed the entire workflow at dispatch with `Unrecognized named-value: 'if'`; the file was locally YAML-valid, and a push produced a *synthetic* failed run attributed to a `push` trigger the workflow does not have. (2) **An empty `matrix` fails the RUN rather than producing no jobs** — with no failed job and no annotation on any check run, so `gh run view` shows every job green under a red run. Reached by any surface whose Linux legs all stay on Azure. Every matrix job is now gated on its own list being non-empty; `[oracle.all]` is the red→green control. (3) **A native command's stdout joins its enclosing PowerShell function's OUTPUT stream**, so `$rc = Invoke-Suite …` returned an array of log lines with the exit code last (measured: 55 elements) and every comparison against it was meaningless — the EF.Core suite never ran on any TFM that ran the main suite, and a main suite exiting 3 produced exit 0. Fixed with `| Out-Host` on every native call.

  **Proving (3) took three fixtures, and the first two failed as controls rather than as tests** — worth recording because both failures are the documented shape. `where.exe` writes to **stderr**, so it never reproduced stdout pollution at all; the replacement used `whoami` as the "passing" suite, but `whoami` with MTP's argument list exits non-zero, so an arm labelled *main passes* never passed. The working fixture needed two purpose-built stub console apps carrying the exact assembly names, because a .NET apphost resolves `<exename>.dll` beside itself and no stock Windows exe exits 0 on an unknown argument list. Only then did the arms disagree: pre-fix `main 3 → exit 0, efcore x0`, post-fix `main 3 → exit 3` and `main 0 → efcore x1`.

  **The flip is held back to a second PR (E-44), on the user's call, and the reason is a silent window.** `tests-comment.yml` always runs the *default branch's* copy of `tests.yml`. So for as long as `testing.yml` says `offload_windows_to_github: true` while master's `tests.yml` has no windows job, a `/azp run test-all` skips those five legs on Azure and cannot run them on GitHub either — they run **nowhere**, and no run goes red to say so. Shipping both halves in one PR would have opened that window on the PR itself; shipping the workflow first closes it by construction, because `offload_windows_to_github` defaults to false and Azure keeps running all thirteen legs until the one-line flip lands. The cost is that phase 4a's own PR proves the five legs only through hand-dispatched `workflow_dispatch` probes, which is what TO-10..TO-13 already are. This is the same ordering #5866 used for the linux legs: workflow first, split afterwards.
- A-6 (2026-09-08) **The linux split shipped in `4cc8a15aa` (PR #5880): 12 legs on GitHub, 3 on Azure**, the three Oracle ones. Both halves were measured on one `test-all` — Azure build 23500 ran exactly `j_Oracle1819` / `q_Oracle2123` / `t_Oracle1112` and 13 windows legs, all green; the companion GitHub run had all 12 `gh_linux` legs green, its only failures being the two Oracle legs the split had just stopped sending there. `b_ClickHouse` moved on evidence (see A-4); the Oracle three stayed after `t_Oracle1112` passed one run and failed the next while `q_Oracle2123` did the reverse — all three flaky on GitHub with the same **ORA-00942** signature, which is isolation damage rather than the cursor exhaustion the container fix addressed, and which re-aims the deferred `.runsettings` experiment at `nunitWorkers` (4 on a GitHub runner against 2 on an Azure agent).

  **This resolves U-2, and it does not resolve the way D-2 assumed.** Per-leg medians (Azure over 10 `test-all` builds, GitHub over 4 runs; failed legs excluded): GitHub is **1.4–1.6x faster** on every linux leg, 574 → 404 min over the 13 comparable ones, and the 12 legs the split moves cost **540 Azure agent-minutes against 371 on GitHub**. Two legs break the pattern for known reasons — `b_ClickHouse` at 1.11x because the `max_threads=2` pin deliberately discards the extra cores, and `t_Oracle1112` at 1.02x because the Oracle legs are container-startup and serialized-DDL bound rather than throughput bound, which is also why retaining them on Azure costs almost no wall clock. Non-test jobs: test-artifacts build 17.1 → 10.3 (1.66x), and the pack-and-verify half 29.1 min as Azure's **single serial job** against a 15.0 min pole across GitHub's six parallel jobs — 1.94x wall clock while spending *more* total agent time (36.4 against 29.1).

  **The consequence for phase 4 is that Azure's critical path is now entirely windows.** Build 23500's 13 windows legs took **1h2m–1h12m each**, against a 67-min pole for GitHub's whole 12-leg linux set. `P10` instructed that D-2's capability-based split be flagged as a load-balancing defect once a full run existed; it now is one, and **E-28** (the `windows-tests` matrix job) is where the remaining wall-clock win lives.
- A-5 (2026-09-07) Withdraws **E-38** in favour of **E-39**. A-4 recorded E-38 as gated on a probe rather than asserted, and the probe refuted it. Build **23453** (`test-sqlite`, chosen because it exercises both arms — populated windows matrix must run, empty linux matrix must skip) finished **green in 52m53s with no `Win` or `Lin` job record at all**, while `Build (win-x86)` and `Create Test Baselines Branch` both succeeded, so every dependency and `enabled` held. The populated windows matrix never ran `s_SQLite`. So `variables['title']` is empty when a **job-level** condition is evaluated; it resolves at step level only, which is where the ~20 existing uses in `test-workflow-*.yml` sit. Applied to every test job the condition turns any `test-all` into a green run executing nothing — the exact shape it was written to prevent, making this the third time D-3's failure mode has been reached by a new route (`containsValue` in A-2, an unforwarded parameter in A-3, a job-level variable here). E-39 fixes it at the level where the variable exists. **Two rules earned, both already in the corpus and both vindicated here:** an Azure expression is only proven in the exact position it is already used in, and a fix whose failure mode is a silent green must not merge on reasoning. The residual cost E-39 accepts is that a job with nothing to do still pays for the unguarded setup steps, `Cleanup Junk` on linux most of all.
- A-4 (2026-09-06) Adds **E-38** and settles the linux split's composition at **12 GitHub / 3 Azure** (PR #5880). The new edit-point is unauthorized by any phase: Azure cannot build a matrix at runtime, so a job whose matrix selects nothing is emitted anyway, runs once with `$(title)` unexpanded, skips every step on its `and(variables.title, …)` guard and reports **succeeded**. That is D-3's recorded failure mode reached by a second route, and it is not hypothetical — build **23319** (`test-clickhouse`) carries a job named literally `Win`, result succeeded, every test step skipped, because `b_ClickHouse` has no windows leg. It predates the migration entirely; the `gh_linux` split only makes the linux side of it common, since every surface but `[all]` and `[oracle.all]` now has all its linux legs on GitHub. **The condition is itself unverified**: whether a matrix variable resolves in a *job-level* condition is undocumented here, and its failure mode is a populated matrix being skipped — a green run with no tests, precisely the class D-3 exists to warn about. Gated on a two-armed probe, `test-sqlite` (windows matrix populated, must run; linux matrix empty, must skip), build 23453.
  ClickHouse moved on evidence rather than on D-2's target. It passed on GitHub with no failures and no retry in 67.0 min against Azure's 72.1 / 72.8 / 74.6 / 77.7 / 73.8 / 74.1 over six builds, and the failure that had held it back — `Issue1813Test11("ClickHouse.Octonica")` dropping its connection after the `max_threads` pin — fired on **Azure** build 23422 on the same code, so Azure was never a safe harbour for it. The three Oracle legs stay: `j_Oracle1819` still exhausts 3000 cursors, and `q_Oracle2123` passed at 10:24 and failed at 14:16. Both now also report **ORA-00942**, a table vanishing mid-test rather than cursor exhaustion — isolation damage, which is what the `nunitWorkers` difference (4 on a GitHub runner against 2 on an Azure agent) would produce, and which re-aims the deferred `.runsettings` experiment at the right variable.
